...
public class View implements Serializable
{
	...
	public ProfileData	m_profile_data;

	public View(ProfileData pd)
	{
		//m_server_data= sd;
		m_profile_data= pd;
	}
	...
	{
		return ServerData.getRawTreeData();
	}
	...
	public boolean changeUserOfVirtualView(VirtualView view, String aUser, boolean hasRight)throws Exception
	{
		...
		IniFile ini= ServerData.getUserIni(aUser); 
		if(ini==null)
	...
	public List<UserEdit> getAllUserEdit()
	{
		if(!isAdmin())
			return null;
		return UserManager.getAllUserEdit(getUserIni(), this, ServerData.getAllUserIni());
	}
	...
	public UserEdit createUserEdit(String loginName) throws Exception
	{
		if(!isAdmin())
			return null;
		Thread.sleep(1000);
		return UserManager.createUserEdit(loginName, getUserIni(), this, ServerData.getAllUserIni());
	}
	/**
	 * 管理员，删除一个用户
	 */
	...
	{
		return m_profile_data.getSpecialLoginName();
	}
	/**
	...
	public IniFile getUserIni()
	{
		...
		return ServerData.getUserIni(LoginName);
	}
	...
	{
		String LoginName= m_profile_data.getLoginName();
		return ServerData.isAdmin(LoginName);
	}
	...
	public boolean setCRUDofNode(INode node) throws Exception
	{
		if(isAdmin())
			return true;
		String LoginName= m_profile_data.getLoginName();
		return ServerData.setCRUDofNode(LoginName, node);
	}
	...
	public SeInfo getSeInfo(INode node)
	{
		...
		e= new SeInfo(node);
		String LoginName= m_profile_data.getLoginName();
		Map<String, String> inivalue= ServerData.getIniValue(LoginName, e.getSvId()); 
		if(inivalue==null)
			return null;
