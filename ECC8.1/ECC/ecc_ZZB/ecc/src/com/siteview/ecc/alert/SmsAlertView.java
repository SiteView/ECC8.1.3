		...
		public void onEvent(Event event) throws Exception {
			try{
				if(view.getStopCount().getValue()<view.getUpdateCount().getValue()){
					Messagebox.show("停止次数不能小于升级次数！", "提示", Messagebox.OK, Messagebox.INFORMATION);
					return;
				}
				if(getAlertReceiver().getValue().equals("其他") && getOtherTelNo().getValue().equals("")){
					Messagebox.show("报警短信接收地址不能为空！", "提示", Messagebox.OK, Messagebox.INFORMATION);
					return;
				}
//				if(!"".equals(getOtherTelNo().getValue())&&!BaseTools.validatePhoneNO(getOtherTelNo().getValue())){
//					Messagebox.show("您输入的手机号码有误，或者暂不支持该手机号！", "提示", Messagebox.OK, Messagebox.INFORMATION);
//					return;
//				}
