	...
	class BatchSelectListener implements EventListener{
		...
		public void onEvent(Event arg0) throws Exception {
			try{
				...
				System.out.println("TYPE 的值:"+ type);
		...
		public void onEvent(Event arg0) throws Exception {
			try{
				...
				for(MonitorTemplateItem type : monitorType){
					...
					String type_str = "";
					if(typeObject!=null){
						type_str = (String)typeObject;
					}
					System.out.println("session中取出的值:"+type_str);
					if(type.id.equals(type_str)){
				...
				//add
				//如何维护 这种标识的状态呢？//遍历循环 如果有 check = true 
