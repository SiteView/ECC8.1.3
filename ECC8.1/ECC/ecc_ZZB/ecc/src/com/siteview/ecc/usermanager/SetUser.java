...
public class SetUser extends GenericAutowireComposer {
	...
	public void onGrant()throws Exception{
		System.out.println(listbox_data.getSelectedItems().size());
		if(listbox_data.getSelectedItems().size()>0){
			final Window win = (Window) Executions.createComponents("/main/setting/role/setGrant.zul", null, null);								
			win.setAttribute("listbox_data", listbox_data);
			win.doModal();
		...
		try{
			...
			if(i==1){
				...
				for(Object it:listbox_data.getSelectedItems()){
					...
					String minfo=loginname+" "+"在"+OpObjectId.user_manage.name+"中进行了  "+OpTypeId.del.name+"操作，删除的用户名为： "+userName;			
					ini.deleteSection(section);
					ini.saveChange();
					AppendOperateLog.addOneLog(loginname, minfo, OpTypeId.del, OpObjectId.user_manage);
				}
				refresh();
			}
		}catch(Exception e){
