...
bool SaveLog(CString strFileName,const CString strLog)
{
	//return false;
...
/*	FILE *pf=fopen(strlog,"a+");
	...
	ret = telnet_init(server, port, proxy_server, proxy_port, proxy_uid, proxy_pwd);
	if(ret <= 0)
	{
		if(ret == -1)
                   ...
                   LPGDATA pgd, struct Tel_Param *pParam)
{
	...
	//WriteLog(strFileName,"=============== telnet_connect:Login ================\n");
...
LOOP:
		...
		while(n--)
		{
			...
			//WriteLog(strFileName, szTemp );

			telnet_protocol(hSocket, *scan++,pgd, pParam);
		}

//		WriteLog(strFileName, "Ω‚Œˆ∫Ûµƒ ˝æ›£∫" );
//		WriteLog(strFileName, pgd->databuffer );

//		strLog.Format("Ω‚Œˆ∫Ûµƒ ˝æ›:\npgd->databuffer=%s",pgd->databuffer);
//		WriteLog(strFileName,strLog);

//		WriteLog(strFileName, "∆•≈‰≤Œ ˝£∫" );
//		WriteLog(strFileName, logprompt );
//		WriteLog(strFileName, pwdprompt );
...
//				    send(hSocket, bufUser, strlen(bufUser),0);
					...
					//WriteLog(strFileName, "”√ªß√˚∆•≈‰≥…π¶£°" );
			    }
            }
            else
            {
                //WriteLogFile("Read Data Failed from Server(1)");
				//WriteLog(strFileName,"Read Data Failed from Server(1)");
                return -7;
            }
			break;
		case 1:
...
//				    send(hSocket, bufPwd, strlen(bufPwd), 0);
					...
					//WriteLog( strFileName,"√‹¬Î∆•≈‰≥…π¶£°" );
			    }
            }
            else
            {
                //WriteLogFile(strFileName,"Read Data Failed from Server(2)");
				//WriteLog(strFileName,"Read Data Failed from Server(2)");
                return -7;
            }
			break;
		case 2:
				  ...
				  char *inifile, int mtype, char *extraparam, char *custpath, char *szReturn)
{
	...
	HANDLE hMutex=NULL;
	CString strMutex=_T(""),strTemp=_T("");
	CString strFile = "MonitorValue";
	CString strLog ="";
	//WriteLog(strFile,"================= MonitorValue Start ================");
	strLog.Format("prompts=%s",prompts);
	...
	CString strFile1  = _T("TELDISKS");
	//CString strLog   = _T("");

	WriteLog(strFile1,"------------- call MonitorValue ---------------");
 ...
 //       pwdprompt, arprompt,pgd , &param)) < 0)
	...
	if( ( mtype == PMEMORY_TYPE_MONITOR ) && 
		( ( strcmp( iniFile, "AIX" ) ==0 ) || 
		  ...
		  ( strcmp( iniFile, "HP" ) == 0 ) ||   //yi.duan add 2012-02-15
		  ( strcmp( iniFile, "sun_china" ) == 0 ) ) )
	{
		...
		//CString strLog = _T("");
		//strFileName.Format("MonitorValue");
		//strLog.Format(" pgd->databuffer:\n%s", pgd->databuffer);
		//SaveLog(strFileName,strLog);
		//////////////////////////////////////////////////////////////////////////
		...
		//////////////////////////////////////////////////////////////////////////
		// lish		
		//SaveLog(strFileName,"============== MonitorValue FREE_PMEMORY ===============");		
		//strLog.Format("buffer=%s",buffer);
		//SaveLog(strFileName,strLog);
		//////////////////////////////////////////////////////////////////////////
		...
		//////////////////////////////////////////////////////////////////////////
		//strLog.Format("pszTemp=%s",pszTemp);
		//SaveLog(strFileName, strLog);

		//SaveLog(strFileName,"============== MonitorValue FREE_PMEMORY  End ===============");
		//////////////////////////////////////////////////////////////////////////
	...
	else if ( (mtype == MEMORYIO_TYPE_MONITOR) &&  
			  ( ( strcmp( iniFile, "Sun" ) == 0 ) || 
			    (strcmp(iniFile, "sun_china") == 0)||
				(strcmp(iniFile, "AIX") == 0) ) )
	{
		...
		strLog.Format("cmd:%s",cmd);

		//WriteLog(strFile1,strLog);
		...
		{
			//OutputDebugString( pgd->databuffer );
			if( !MONITOR_PARSE( MEMORYIO_TYPE_MONITOR, pgd->databuffer, inifile, extraparam, buffer ) )
			{
	...
	else if ((mtype == MEMORYIO_TYPE_MONITOR) && (strcmp(iniFile, "HP") == 0))
	{
		char buffer[1024] = {0};
		try
		{
		...
		{
			//OutputDebugString( pgd->databuffer );
			if( !MONITOR_PARSE( MEMORY_TYPE_MONITOR, pgd->databuffer, inifile, extraparam, buffer ) )
			{
				printf("parse error\n");
				sprintf(szReturn, "error=%s", FuncGetStringFromIDS("SV_BASIC",
					"BASIC_PARSER_STRING_FAILED"));// <%IDS_TelMonitor_16%>
		...
		{
			cout<<"exception occured,error number"<<::GetLastError()<<endl;
			bResult=false;
			goto err;
		}
		CString strTemp = _T("");
	...
	*/
	...
	//    WriteLogFile(cmd);
		...
		if((ret = telnet_command(hSocket, cmd,pgd, &param)) < 0)
		{
			...
			WriteLog(strFile1,"------------ ªÒ»° ˝æ› ß∞‹£°--------------");
			goto err;
		}
	
		printf("pgd->databuffer:\n%s\n",  pgd->databuffer);
...
///------------------bin.liu----------------
///
bool ZHWMonitorValue(char *server, int port, char *uid, char *pwd, 
				  char *proxy_server, int proxy_port, char *proxy_uid, char *proxy_pwd, 
				  char *logprompt, char *pwdprompt, char *prompts, 
				  char *inifile, int mtype, char *strMatchStr,char *strMatchALLStr, char *custpath, char *szReturn)
{
	WriteLog( "================= MonitorValue ================" );
...
//	{
	...
	*/
	...
	//	puts(pgd->databuffer);
		...
		{		
			OutputDebugString(pgd->databuffer); //zjw
			//if(!MONITOR_PARSE(mtype, pgd->databuffer, inifile, strMatchStr, szReturn))
			//{
			//	printf("parse error\n");
			//	sprintf(szReturn, "error=%s", FuncGetStringFromIDS("SV_BASIC",
			//		"BASIC_PARSER_STRING_FAILED"));   //  <%IDS_TelMonitor_16%>
			...
			//	goto err;
			//}
			if(!MONITOR_PARSE_Process(strMatchStr, pgd->databuffer, inifile, strMatchALLStr, szReturn))
			{
				printf("parse error\n");
				sprintf(szReturn, "error=%s", FuncGetStringFromIDS("SV_BASIC",
					"BASIC_PARSER_STRING_FAILED"));   //  <%IDS_TelMonitor_16%>
