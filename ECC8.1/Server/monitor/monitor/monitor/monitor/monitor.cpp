...
BOOL SSH_AIX_ERRPT_LIST(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""), 
...
extern "C" __declspec(dllexport) 
BOOL AIXERRPTLIST(const char *strParas, char * strReturn, int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
	case 1:
		bRBool = SSH_AIX_ERRPT_LIST(paramList, szReturn);
		break;
	//case 2:
	//	bRBool= TEL_AIX_ERRPT_LIST(paramList,  szReturn,nSize);
	//	break;
	default:
		return FALSE;
	}
	...
	//if(bRBool&&nSize>lsize*2)
	{
		...
		while(1)
		{
			...
			if(cb = strstr(ca, "$$$"))
			{
				...
				if(strlen(proc)>0)  
				{
					if (strrchr(proc, '=') != NULL)        //dy当返回数据中有“=”。 用&
					{
						sprintf(sa,"%s&%s",proc,proc);     //dy
					}
					else				
					{
...
BOOL SSH_AIX_ERRPT_DETAIL(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
	...
	while(pos)
	{
		...
		else if(strTemp.Find("_errorName=", 0) == 0)
		{
			strErrorName = strTemp.Right(strTemp.GetLength() - strlen("_errorName="));
		}
		else if(strTemp.Find(__CUSTOMERPATH__, 0) == 0)
		{
			strCustomerPath.Format("%s\\%s\\temp", FuncGetInstallRootPath(), strTemp.Right(strTemp.GetLength() - strlen(__CUSTOMERPATH__)));
	...
	bool ret = SSHMonitorValue(	strServer.GetBuffer(strServer.GetLength()), 
		...
		AIX_ERRPT_LIST_MONITOR, 
		strIniFile.GetBuffer(strIniFile.GetLength()), 
		strErrorName.GetBuffer(strErrorName.GetLength()), 
		strCustomerPath.GetBuffer(strCustomerPath.GetLength()),
		strPrivateKeyFile.GetBuffer(strPrivateKeyFile.GetLength()),
		szReturn);
...
extern "C" __declspec(dllexport) 
bool AIXERRPTDETAIL(const char *strParas,  char *szReturn,int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
	case 1:
		bRet= SSH_AIX_ERRPT_DETAIL(paramList, szReturn);
		break;
	//case 2:
...
BOOL CHECKPOINT(const char *strParas,  char *szReturn,int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	cout << "--------------- CHECKPOINT  Start ------------------"<<endl;
	CString		strServer = _T(""), 
				strCommunity = _T("public"), 
				strIndex = _T("0");
	int			nPort = 161, 
	...
	cout << "--------------- FuncGetInstallRootPath ---------------- "<<endl; 
	strCustomerPath.Format("%s\\temp", FuncGetInstallRootPath());

	cout << "----------------- Find Params start ------------"<<endl;
	CStringList paramList;
	MakeStringListByChar(paramList,strParas);
	POSITION pos = paramList.FindIndex(0);
	while(pos)
	...
	cout << "--------------- Call CHECKPOINT_MONITOR  ------------------"<<endl;
	...
	WSA_Free();
	cout << "--------------- Call CHECKPOINT_MONITOR end  ------------------"<<endl;
	return bResult;
}
...
BOOL IPLANET(const char * strParas, char * szReturn, int &nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
BOOL SSHLOGFILE(CStringList &paramList, char *szReturn)
{
	...
	BOOL ret = LOGFILE_SSH_VALUE_IMPROVE(strServer.GetBuffer(strServer.GetLength()), 
								nPort, 
								strUID.GetBuffer(strUID.GetLength()), 
								strPWD.GetBuffer(strPWD.GetLength()), 
								strIniFile.GetBuffer(strIniFile.GetLength()), 
...
//	WriteLog(strFile ,"----------------- Call NETSCREENATTACK ------------------");
...
//	WriteLog(strFile, "--------------------- NETSCREENATTACK  End ----------------");
	WSA_Free();
...
bool TRAP( const char *strSrc,  char *szReturn, int& nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
BOOL TELCPUs(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
BOOL SSHCPUs(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
//	cout << "------------------- Call CPU Function------------------" << endl;
	...
	while(pos)
	{
		...
		{
			nProtoType =atoi(strTemp.Right(strTemp.GetLength() - strlen(__PROTOCOLTYPE__)));
			cout << "nProtoType:" << nProtoType <<endl;
			break;
		}
	}

	if(nProtoType == 0) nProtoType = 1;
...
//
extern "C" __declspec(dllexport) 
bool CPUs( const char *strSrc,  char *szReturn, int& nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	//	cout << "------------------- Call CPUs Function------------------" << endl;
	...
	{
	case 1:
		bRet= SSHCPUs(paramList, szReturn);
		break;
	case 2:
		bRet= TELCPUs(paramList, szReturn);
		break;
	/*case 3: unix没这2种连接方式
		bRet = HTTPCPU(paramList, szReturn);
		break;
	...
	if( ( nstyle == PMEMORY_TYPE_MONITOR ) && 
		( ( strcmp( strIniFile.GetBuffer(strIniFile.GetLength()), "AIX" ) == 0 ) || 
		  ( strcmp( strIniFile.GetBuffer(strIniFile.GetLength()), "Sun" ) == 0 ) || 
		   ( strcmp( strIniFile.GetBuffer(strIniFile.GetLength()), "HP" ) == 0 ) || 
...
BOOL SSH_AIX_MEMORY(CStringList &paramList, char *szReturn,int nstyle)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
//查看swap分区
extern "C" __declspec(dllexport) 
BOOL SWAP(const char *strParas,  char *szReturn,int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
	case 1:
		bRet= SSHMEMORY(paramList, szReturn,SWAP_TYPE_MONITOR);
		break;
	case 2:
		bRet= TELMEMORY(paramList, szReturn,SWAP_TYPE_MONITOR);
		break;
	default:
		sprintf(szReturn, "error=%s", FuncGetStringFromIDS("SV_BASIC",
			"BASIC_PROTOCOL_TYPE_ERROR"));//<%IDS_Monitor_148%>"协议类型错误"
...
//取AIX内存 svmon -G //yi.duan 2011-06
extern "C" __declspec(dllexport) 
BOOL AIXMEMORY(const char *strParas,  char *szReturn,int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
	case 1:
		bRet= SSH_AIX_MEMORY(paramList, szReturn,MEMORY_TYPE_MONITOR);
		break;
	case 2:
		bRet= TELMEMORY(paramList, szReturn,MEMORY_TYPE_MONITOR);
		break;
...
bool SSHSERVICE(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
				...
				strService = _T(""),
				strPrivateKeyFile = _T("");
	//CString		strMatchCharacters = _T(""); //add yi.duan 2012-02-17
	//CString		strMonitorId = _T(""); //add yi.duan 2012-02-17
	int			nPort = 22; 
	...
	while(pos)
	{
		...
		{
			strCustomerPath = strTemp.Right(strTemp.GetLength() - strlen(__CUSTOMERPATH__));
			//strCustomerPath.Format("%s\\%s\\temp", FuncGetInstallRootPath(), strTemp.Right(strTemp.GetLength() - strlen(__CUSTOMERPATH__)));
		}
		else if(strTemp.Find("_PriKeyPath=", 0) == 0)
		{
		...
		{
			strMonitorId = strTemp.Right(strTemp.GetLength() - strlen("_MonitorID="));
		}
		else if(strTemp.Find("_codeList=", 0) == 0)
		{
			strMatchCharacters = strTemp.Right(strTemp.GetLength() - strlen("_codeList="));
		}*/
	}
	...
	//if (!(strMatchCharacters.IsEmpty()))
	//{
	//	//char CurrPath[1024] = {0};
	//	//char fileName[1024] = {0};
	//	//GetCurrentDirectory(sizeof(CurrPath)-1, CurrPath);
	//	//sprintf(fileName, "%s\\%s.ini", CurrPath, strServer.GetBuffer(strServer.GetLength()));
	//	//printf("fileName = %s\n",fileName);
	//	//printf("_codeList = %s\n",strMatchCharacters.GetBuffer(strMatchCharacters.GetLength()));
	//	//WritePrivateProfileString(strMonitorId.GetBuffer(strMonitorId.GetLength()), "matchStr",
	//	//							strMatchCharacters, fileName);

	//	printf("strCustomerPath = %s\n",strCustomerPath.GetBuffer(strCustomerPath.GetLength()));
	//	strCustomerPath = strMatchCharacters;
	//	strCustomerPath.ReleaseBuffer();
...
void MakeDisksReturn(char * szReturn,int& nSize )
{
	...
	while(lsResult.size()>0 && lsize<nSize)
	{
		...
		CString strFile = _T("MakeDisksReturn");
		WriteLog(strFile , strTmp.c_str());
		lsResult.pop_front();
...
bool TELDISKS(CStringList &paramList, char *strReturn,int nSize)
{
	...
	CString strFile  = _T("TELDISKS");
	CString strLog   = _T("");

	WriteLog(strFile,"------------- call TELDISKS ---------------");
	CString		strCustomerPath = _T("");
	strCustomerPath.Format("%s\\temp", FuncGetInstallRootPath());
	...
	else 
    {
		...
		WriteLog(strFile, "--------------- TELDISKS Success -------------------");
	}
...
bool SSHDISKS(CStringList &paramList, char *strReturn,int nSize)
{
	...
	else 
    {
		...
		//lish
		CString strFileName = "";
		strFileName.Format("SSHDISKS");
		WriteLog(strFileName,szReturn);
		//////////////////////////////////////////////////////////////////////////
		sprintf(strReturn,szReturn);
		MakeDisksReturn(strReturn,nSize);
		WriteLog(strFileName,strReturn);
	}
...
bool  DISKS(const char *strParas, char * strReturn, int & nSize)
{
	...
	default:
		sprintf(strReturn,"error=Error occurred: Protocol type is invalidation$");
		return false;
	}
	//MakeDisksReturn(strReturn,nSize);
	return true;
...
extern "C" __declspec(dllexport) 
bool  MYDISKS(const char *strParas, char * strReturn, int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
	case 1:
		bRet = SSHDISKS(paramList, strReturn,nSize);
		break;
	case 2:
		bRet = TELDISKS(paramList, strReturn,nSize);
		break;
		//case 3:
		//	return HTTPDISKS(paramList, lstTexts, lstValues);
		//case 4:
	...
	//MakeDisksReturn(strReturn,nSize);
	int nTest = strlen(strReturn);
	sprintf(strReturn,"TestValue=%d$",nTest);
...
void MakeCpusReturn(char* szOut, char* szIn, int& nSize )
{
	char *ca = NULL;
	char *cb = NULL;
	char *pOut = szOut;
...
bool SSHMultiCPUS(CStringList &paramList, char *strReturn,int nSize)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
	...
	CString strFile=_T("SSHMultiCPUS");
	CString strLog=_T("");
	...
	char szReturn[8912]={0};

	bool bResult = SSHMonitorValue(strServer.GetBuffer(strServer.GetLength()), 
									nPort, 
									strUID.GetBuffer(strUID.GetLength()), 
									strPWD.GetBuffer(strPWD.GetLength()), 
									MULCPUS_TYPE_MONITOR, 
									strIniFile.GetBuffer(strIniFile.GetLength()), 
									NULL, 
									strCustomerPath.GetBuffer(strCustomerPath.GetLength()), 
									strPrivateKeyFile.GetBuffer(strPrivateKeyFile.GetLength()),
	...
	else 
	{
		...
		//sprintf(strReturn,szReturn);
		//MakeDisksReturn(strReturn,nSize);
		MakeCpusReturn(strReturn,szReturn,nSize);
	}
...
bool TELMultiCPUS(CStringList &paramList, char *strReturn,int nSize)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
	...
	else 
	{
		...
		//lish
		CString strFileName = "";
		strFileName.Format("TELMultiCPUS");
		WriteLog(strFileName,szReturn);
		//////////////////////////////////////////////////////////////////////////
		sprintf(strReturn,szReturn);
		//MakeDisksReturn(strReturn,nSize);
...
//获取多核CPU序列
extern "C" __declspec(dllexport) 
bool  MultiCPUS(const char *strParas, char * strReturn, int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
	case 1:
		bRet = SSHMultiCPUS(paramList, strReturn, nSize);
		break;
	case 2:
		bRet = TELMultiCPUS(paramList, strReturn, nSize);
		break;
	default:
		sprintf(strReturn,"error=Error occurred: Protocol type is invalidation$");
		return false;
...
void MakeCupValue(CString strMonitorID, char *szOut, char *szIn, int& nSize  )
{
	unsigned int nUserTime, nSystemTime, nIdleTime;
	CString strUser, strSystem, strIdle;
	char *ca = szIn;
	char *cb = NULL;
	std::list<string> 	lstPairs;
	while(1)
	...
	for(std::list<string>::iterator It= lstPairs.begin();It!=lstPairs.end();It++)
	{
		...
		if (cb)
		{
			...
			{
				strUser = cb+1;
				nUserTime= atoi(strUser);
			}
			else if(strstr(ca, "system"))
			{
				strSystem = cb +1;
				nSystemTime = atoi(strSystem);
			}
			else if (strstr(ca, "idle"))
			{
				strIdle = cb+1;
				nIdleTime = atoi(strIdle);
			}
			else
			{
	...
	CString strFile = _T("");
	strFile.Format("..\\data\\TmpIniFile\\Unix_%s.ini", strMonitorID);
	unsigned int nPreUser, nPreSystem, nPreIdle;
	char szPreUser[64]={0};
	GetPrivateProfileString(strMonitorID, "userTime", "", szPreUser, 63, strFile);
	char szPreSystem[64]={0};
	GetPrivateProfileString(strMonitorID, "systemTime", "", szPreSystem, 63, strFile);
	char szPreIdle[64]={0};
	GetPrivateProfileString(strMonitorID, "idleTime", "", szPreIdle, 63, strFile);
	char szLastUtilization[64]={0};
	GetPrivateProfileString(strMonitorID, "Utilization", "", szLastUtilization, 63, strFile);

	WritePrivateProfileString(strMonitorID, "userTime",  strUser, strFile);
	WritePrivateProfileString(strMonitorID, "systemTime",  strSystem, strFile);
	WritePrivateProfileString(strMonitorID, "idleTime",  strIdle, strFile);

	float fUtilization = 0.00;
	if ( strlen(szLastUtilization) == 0 )
	{
		fUtilization = 100.00*(nUserTime+nSystemTime)/(nUserTime+nSystemTime+nIdleTime);
	}
	else
	{
		nPreIdle	= atoi(szPreIdle);
		nPreUser	= atoi(szPreUser);
		nPreSystem	= atoi(szPreSystem);
        nIdleTime	-= nPreIdle;
		nUserTime	-= nPreUser;
		nSystemTime -= nPreSystem;

		fUtilization = 100.00*(nUserTime+nSystemTime)/(nUserTime+nSystemTime+nIdleTime);
	}
	if (fUtilization < 0.00 )
	{
		fUtilization = 0.00;
	}
	else if (fUtilization > 100.00)
	{
		fUtilization = 100.00;
	}
	
	CString strTemp=_T("");
	strTemp .Format("%.2f",fUtilization);
	WritePrivateProfileString(strMonitorID, "Utilization",  strTemp, strFile);
	sprintf(szOut,"utilization=%.2f",fUtilization);
}

bool SSHMultiCPU(CStringList &paramList, char *strReturn,int nSize)
{
	CString		strServer = _T(""),
				strUID = _T(""),
				strPWD = _T(""),
	...
	CString strFile=_T("SSHMultiCPU");
	CString strLog=_T("");
	...
	while(pos)
	{
		...
		else if (strTemp.Find(__CPUNUM__,0) == 0)
		{
			strCPU = strTemp.Right(strTemp.GetLength() - strlen(__CPUNUM__));
		}
		else if (strTemp.Find(__MONITORID__,0) == 0)
		{
			strMonitorID = strTemp.Right(strTemp.GetLength() - strlen(__MONITORID__));
	...
	bool bResult = SSHMonitorValue(strServer.GetBuffer(strServer.GetLength()), 
									...
									MULCPU_TYPE_MONITOR, 
									strIniFile.GetBuffer(strIniFile.GetLength()), 
									strCPU.GetBuffer(strCPU.GetLength()), 
									strCustomerPath.GetBuffer(strCustomerPath.GetLength()), 
									strPrivateKeyFile.GetBuffer(strPrivateKeyFile.GetLength()),
									szReturn);
	...
	if (!bResult)
	{
		sprintf(strReturn,"%s",szReturn);
		return false;
	}
	else
	{
		MakeCupValue( strMonitorID,strReturn, szReturn, nSize);
	}
...
bool TELMultiCPU(CStringList &paramList, char *strReturn, int nSize)
{
	CString		strServer = _T(""),
				strUID = _T(""),
				strPWD = _T(""),
	...
	if(strServer.IsEmpty())
	{
		sprintf(strReturn, "error=%s", FuncGetStringFromIDS("SV_UNIX_SERVER",
			"UNIX_SERVER_IP_ADDRESS_NULL"));//<%IDS_Monitor_134%>"缺少服务器"
		return false;
	}
	if(nPort == 0)
	...
	if(strLogPrompt.IsEmpty() || strPwdPrompt.IsEmpty() || strPrompt.IsEmpty())
	{
		sprintf(strReturn, "error=%s", FuncGetStringFromIDS("SV_UNIX_SREVER",
			"UNIX_SERVER_PROMPT_NULL"));//<%IDS_Monitor_132%>"输入登录提示符、密码提示符或提示符"
		return false;
	}
	...
	if(WSA_Init())
	{
		sprintf(strReturn, "error=%s", FuncGetStringFromIDS("SV_BASIC",
			"BASIC_WSA_INIT_FAILED"));//<%IDS_Monitor_137%>"通信初始化错误"
		return false;
	}
	...
	WSA_Free();
	CString strFile=_T("TELMultiCPU");
	WriteLog(strFile, szReturn);
	if (!ret)
	{
		sprintf(strReturn,"%s",szReturn);
	...
	//sprintf(strReturn,"utilization=2.00\0\0");
	return ret;
}
...
//获取指定cpu的利用率
extern "C" __declspec(dllexport) 
bool  MultiCPU(const char *strParas, char * strReturn, int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
	case 1:
		bRet = SSHMultiCPU(paramList, strReturn, nSize);
		break;
	case 2:
		bRet = TELMultiCPU(paramList, strReturn, nSize);
		break;
	default:
		sprintf(strReturn,"error=Error occurred: Protocol type is invalidation$");
		return false;
...
extern "C" __declspec(dllexport) 
BOOL MEMORY_WH(const char *strParas,  char *szReturn,int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
BOOL SSHMEMORYIO(CStringList &paramList, char *strReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
BOOL TELMEMORYIO(CStringList &paramList, char *strReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
extern "C" __declspec(dllexport)    //dy注释，换top命令，只需1条命令,有机会试试
BOOL MEMORYINFO(const char *strParas,  char *strReturn,int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	while(pos)
	{
		...
		{
            strOsType = strTemp.Right(strTemp.GetLength() - strlen(__OSTYPE__));
		}
	}

	//cout << "strOsType:" << strOsType << endl;

	if(nProtoType == 0) nProtoType = 2;

	char szReturn1[1024] = {0};
	char szReturn2[1024] = {0};
	CString strTemp=_T("");
	bool bRet;
	switch(nProtoType)
	{
	case 1:
		{
			bRet= SSHMEMORYIO(paramList, szReturn1);
			if ( strOsType.Compare("Sun") == 0		|| 
				strOsType.Compare("sun_china") == 0	||
				strOsType.Compare("AIX") == 0	
				)
			{
			...
			{ 
				...
				bRet = SSHMEMORY(paramList, szReturn2, MEMORY_TYPE_MONITOR);
				cout << "szReturn1:"<<szReturn1<<endl;
				cout << "szReturn2:" <<szReturn2<<endl;
				if (bRet)
				{
					strTemp = szReturn1;
					strTemp += szReturn2;
	...
	case 2:
		{
			bRet= TELMEMORYIO(paramList, szReturn1);
			if ( strOsType.Compare("Sun") == 0 || 
				strOsType.Compare("sun_china") == 0  ||
				strOsType.Compare("AIX") == 0)
			{
			...
			if (bRet)
			{
				bRet = TELMEMORY(paramList, szReturn2, MEMORY_TYPE_MONITOR);
				if (bRet)
				{
					strTemp = szReturn1;
					strTemp += szReturn2;
	...
	sprintf(strReturn,"%s$",strTemp);
	CString strTmp=strReturn;
	MakeCharByString(strReturn,nSize,strTmp);	

	return bRet;

}

BOOL SSHSystemRun(CStringList &paramList,  char * strReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
BOOL TELSystemRun(CStringList &paramList,  char * strReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
extern "C" __declspec(dllexport) 
BOOL GetSystemRun(const char *strParas,  char *strReturn,int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
	case 1:
		bRet= SSHSystemRun(paramList, strReturn);
		break;
	case 2:
		bRet= TELSystemRun(paramList, strReturn);
		break;
	default:
		sprintf(strReturn, "error=%s", FuncGetStringFromIDS("SV_BASIC",
			"BASIC_PROTOCOL_TYPE_ERROR"));//<%IDS_Monitor_138%>"协议类型错误"
	...
	CString strTmp=strReturn;
	//sprintf(strReturn,"systemRunTime=0.00$systemIdleTime=0.00$systemRunLoad=0.00$$");
	//strTmp=strReturn;
	MakeCharByString(strReturn,nSize,strTmp);
...
bool SSHPROCESS(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
bool TELPROCESS(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
	...
	strPrompt.TrimRight();
	bool ret = true;
	//printf("%s -%s ", strUID.GetBuffer(strUID.GetLength()), strPWD.GetBuffer(strPWD.GetLength()));
	try
	{
		ret = MonitorValue(strServer.GetBuffer(strServer.GetLength()), 
							nPort, 
...
extern "C" __declspec(dllexport) 
bool PROCESS(const char *strParas,  char *szReturn,int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
	case 1:
		bRet= SSHPROCESS(paramList, szReturn);
		break;
	case 2:
		bRet= TELPROCESS(paramList, szReturn);
		break;
	default:
		sprintf(szReturn, "error=%s", FuncGetStringFromIDS("SV_BASIC",
			"BASIC_PROTOCOL_TYPE_ERROR"));//<%IDS_Monitor_172%>"协议类型错误"
	...
	//sprintf(szReturn,"ProcessOccupyCPUTime=0.00$PhysicalMemory=0.00$ProcessStartTime=14.57$ProcessStatus=S$Processes=1$$");
...
BOOL TELPROCESSES(CStringList &paramList, char *szReturn,int nSize)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
BOOL SSHPROCESSES(CStringList &paramList,  char *szReturn,int nSize)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
extern "C" __declspec(dllexport) 
BOOL PROCESSES(const char *strParas, char * strReturn, int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	BOOL bRBool;
	char szReturn[1024*500] = {0};
	switch(nProtoType)
	{
	case 1:
		bRBool= SSHPROCESSES(paramList, szReturn,nSize);
		break;
	case 2:
		bRBool= TELPROCESSES(paramList,  szReturn,nSize);
		break;
	default:
		return FALSE;
	}
...
BOOL TELDISKIO(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
BOOL SSHDISKIO(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
extern "C" __declspec(dllexport) 
BOOL DISKIO(const char *strParas, char * strReturn, int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
	case 1:
		bRet = SSHDISKIO(paramList, strReturn);
		break;
	case 2:
		bRet = TELDISKIO(paramList, strReturn);
		break;
		//case 3:
		//	return HTTPDISKS(paramList, lstTexts, lstValues);
		//case 4:
	...
	//sprintf(strReturn,"ReadPerSec=0.00$WritePerSec=0.00$TransfersPerSec=0.00$$");
...
bool SSHPROCESSNUM(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
bool TELPROCESSNUM(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
extern "C" __declspec(dllexport) 
BOOL PROCESSNUM(const char *strParas, char * szReturn, int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
	case 1:
		bRet= SSHPROCESSNUM(paramList, szReturn);
		break;
	case 2:
		bRet= TELPROCESSNUM(paramList, szReturn);
		break;
	default:
		sprintf(szReturn, "error=%s", FuncGetStringFromIDS("SV_BASIC",
			"BASIC_PROTOCOL_TYPE_ERROR"));//<%IDS_Monitor_172%>"协议类型错误"
...
//yi.duan 国家绘测局 2010-12-08
BOOL SSHArcsdeUser(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
BOOL TELArcsdeUser(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
		strUID = _T(""),
		strPWD = _T(""),
...
//国家绘测局 定值 ARCSDE 2010-12-08 yi.duan
extern "C" __declspec(dllexport) 
bool ArcsdeUser( const char *strSrc,  char *szReturn, int& nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	//	cout << "------------------- Call CPU Function------------------" << endl;
	...
	{
	case 1:
		bRet= SSHArcsdeUser(paramList, szReturn);
		break;
	case 2:
		bRet= TELArcsdeUser(paramList, szReturn);
		break;
	default:
		sprintf(szReturn, "error=%s", FuncGetStringFromIDS("SV_BASIC",
			"BASIC_PROTOCOL_TYPE_ERROR"));//<%IDS_Monitor_138%>"协议类型错误"
...
bool ZHWSSHSERVICE(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
				strUID = _T(""),
				strPWD = _T(""),
				...
				strService = _T(""),
				strPrivateKeyFile = _T("");
	CString		strMatchStr = _T(""); //模糊匹配的字符
	CString		strMatchALLStr = _T(""); //全匹配的字符串
	int			nPort = 22; 
	...
	while(pos)
	{
		...
		{
			strMatchStr = strTemp.Right(strTemp.GetLength() - strlen("_codeList="));
		}
		else if(strTemp.Find("_processName=", 0) == 0)
		{
			strMatchALLStr = strTemp.Right(strTemp.GetLength() - strlen("_processName="));
		}
		/*else if (strTemp.Find("_MonitorID=", 0) == 0)
		{
			strMonitorId = strTemp.Right(strTemp.GetLength() - strlen("_MonitorID="));
 ...
 //           "UNIX_SERVICE_NAME_NULL"));//<%IDS_Monitor_170%>"缺少服务名称"
	...
	bool ret = ZHWSSHMonitorValue(strServer.GetBuffer(strServer.GetLength()), 
								nPort, 
								strUID.GetBuffer(strUID.GetLength()), 
								strPWD.GetBuffer(strPWD.GetLength()), 
								SERVICE_TYPE_MONITOR, 
								strIniFile.GetBuffer(strIniFile.GetLength()), 
								strMatchStr.GetBuffer(strMatchStr.GetLength()), 
								strMatchALLStr.GetBuffer(strMatchALLStr.GetLength()), 
								strCustomerPath.GetBuffer(strCustomerPath.GetLength()),
								strPrivateKeyFile.GetBuffer(strPrivateKeyFile.GetLength()),
								szReturn);
...
bool ZHWTELSERVICE(CStringList &paramList, char *szReturn)
{
	CString		strServer = _T(""),
				strUID = _T(""),
				strPWD = _T(""),
 ...
 //           "UNIX_SERVICE_NAME_NULL"));//<%IDS_Monitor_164%>"缺少服务名称"
    ...
    try
    {
	    ret = ZHWMonitorValue(strServer.GetBuffer(strServer.GetLength()), 
							nPort, 
							strUID.GetBuffer(strUID.GetLength()), 
							strPWD.GetBuffer(strPWD.GetLength()), 
							strProxyServer.GetBuffer(strProxyServer.GetLength()), 
...
extern "C" __declspec(dllexport) 
bool ZHWSERVICE(const char *strParas,  char *szReturn,int & nSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	{
	case 1:
		bRet= ZHWSSHSERVICE(paramList, szReturn);
		break;
	case 2:
		bRet= ZHWTELSERVICE(paramList, szReturn);
		break;
	//case 3:
	//	return HTTPSERVICE(paramList, szReturn);
	//case 4:
