...
bool SaveLog(CString strFile, CString strLog)
{
	...
	szFileName.Format("%s\\%s.log",szFileName,strFile);
	//szFileName.Format("%s.log",strFile);
			  ...
			  struct oid *oidp, struct snval **ppsnval)
{
	...
	CString strLog = "";
	CString strTemp="";
	strFile.Format("SN_VALUES");
	SaveLog(strFile,"----------------------- SN_VALUES Start --------------");
	cout << "---------- SN_VALUES Start --------------" <<endl;
	...
	ver[0] = ASN1_INT;
	verlen = 1 + a1writeint(SNMP_VERSION - 1, ver + 1, ASN1_INT);
	strLog.Format("verlen = %d",verlen);
	SaveLog(strFile,strLog);
	...
	strLog.Format("commlen = %d, tmp=%d",commlen,tmp);
	SaveLog(strFile,strLog);
	...
	strLog.Format("reqidlen=%d, tmp=%d",reqidlen,tmp);
	SaveLog(strFile,strLog);
	...
	errstatus[0] = ASN1_INT;
	errstatuslen = 1 + a1writeint(0, errstatus + 1, ASN1_INT);
	strLog.Format("errstatuslen=%d",errstatuslen);
	SaveLog(strFile,strLog);
	...
	errindex[0] = ASN1_INT;
	errindexlen = 1 + a1writeint(0, errindex + 1, ASN1_INT);
	strLog.Format("errindexlen=%d",errindexlen);
	SaveLog(strFile,strLog);
	...
	while(poid)
	{
		...
		strLog.Format("nCount=%d,objlen=%d",nCount,objlen);
		SaveLog(strFile,strLog);
		...
		strLog.Format("before objvallen=%d",objvallen);
		SaveLog(strFile,strLog);
		...
		for(i = 0;i < objlen;i ++)
			objval[objvallen ++] = object[i];
		strLog.Format("after objvallen=%d",objvallen);
		SaveLog(strFile,strLog);
		
		strLog.Format("poid->name :%s",poid->name);
		SaveLog(strFile,strLog);
	...
	for(i = 0; i < objvallen; i ++)
		objects[objectslen ++] = objval[i];
	strLog.Format("objectslen=%d,tmp=%d",objectslen,tmp);
	SaveLog(strFile,strLog);
...
//	WriteLog(strFile,strLog);
	...
	//WriteLog(strFile,"=============== SN_VALUES ================");
	//strLog.Format("verlen=%d, ver=%s",verlen,ver);
	//WriteLog(strFile, strLog);
	cout << "verlen = "<<verlen<<endl;
	cout << "commlen = "<<commlen <<endl;
	cout << "pdulen = " <<pdulen<<endl;
	cout << "requestlen = "<< requestlen <<endl;
	//strLog.Format("commlen = %d,comm=%s",commlen,comm);
	//WriteLog(strFile, strLog);

	//strLog.Format("pdulen = %d, pdu=%s",pdulen,pdu);
	//WriteLog(strFile, strLog);
	...
	//strLog.Format("requestlen=%d\nrequest=%s",requestlen,request);
	//WriteLog(strFile,strLog);
	
	cout << "---------------- sendto start -----------------" <<endl;
w:	n = sendto(s, (char *)request, requestlen, 0, (struct sockaddr *)&serv, sizeof(serv));
	if(n == SOCKET_ERROR)
	{
		return -7;
	}
	cout << "------------------ RESPONSE_WAIT -----------------" <<endl;
	if(RESPONSE_WAIT(s, SNMP_WAIT_TIMEOUT) > 0)
	{
		dw = sizeof(serv);
		n = recvfrom(s, (char *)reply, 1000, 0, (struct sockaddr *)&serv, &dw);
...
//		WriteLog(strFile,strLog);
		...
		cout << "--------------------- SN_PARSE_VALUES start --------------------"<<endl;
		SN_PARSE_VALUES(reply, n, &index, &field, &errstat, NULL, ppsnval);

		cout << "errstat="<<errstat <<endl;

		char strTempErr[32];
		sprintf(strTempErr,"errstat=%d",errstat);
		WriteLog(strTempErr);
//		WriteLog(strFile,strTempErr);
		
//		WriteLog(strFile,"----------  End 1 ---------------");
					 ...
					 struct oid *poid, struct snval **ppsnval)
{
	...
	CString strFile = "SN_PARSE_VALUES";
	CString strLog = "";
	int oldIndex;
	...
	while(*index < plen)
	{
		...
		case ASN1_INT:
			...
			case 3:
				...
				break;
			case 4:
//				WriteLog(strFile,"================ ERROR ======================");
				*errstatus = ivalue;
				strLog.Format("tmp=%d, index=%d,lenlen=%d\npack:\n%s",tmp, oldIndex,lenlen,pack);
//				WriteLog(strFile, strLog);
#ifdef __DEBUG__
...
int MakeSocket(char *server, int port, int &s, struct sockaddr_in &serv)
{
	...
	CString strFile = "";
	CString strFile1 ="NETSCREENATTACK";
	CString strLog ="";
	strFile.Format("MakeSocket_%s",server);
	...
	while(hp->h_addr_list[i] != NULL)
	{
		...
		strLog.Format("hp->h_addr_list_%d=%s",i,hp->h_addr_list[i]);
//		WriteLog(strFile,strLog);
