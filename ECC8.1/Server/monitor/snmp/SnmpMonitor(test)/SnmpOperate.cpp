...
bool CSnmpOperate::ReadConfig()
{
	...
	{//读取每一条OID纪录
		...
		sprintf(key1, "oidnum_string%d", i + 1);
		pOidList[i].bString = FuncGetProfileIntBy(chSection, key1, chSMIFile);
...
bool CSnmpOperate::GetResult(char* chReturn, int & nSize)
{
	...
	{//初始化SNMP 
		...
		map<string, int> mapOidType;// 保存oid的类型：0 简单变量 1 表格变量
		map<string,MonitorResult> mapOid;// 保存oid以及结果
		map<string,MonitorResult>::iterator itMRet;
		map<string,string> mapStringRet;
		...
		{//得到每一个oid的结果
            ...
            {                
                sprintf(chOID, "%s", pOidList[i].chOID);
				//sprintf(chOID, "%s.0", pOidList[i].chOID);// 这条语句才是正确的，但是原来的smi.ini中很多都是直接将0写到了oid中，所以只能将错就错
            }

			MonitorResult resultList;

			mapOid.insert( pair<string,MonitorResult>(chOID, resultList));
			mapOidType.insert( pair<string,int>(chOID, nOidType));
		}


		// 获取oid的值
		for( itMRet=mapOid.begin(); itMRet!=mapOid.end(); itMRet++ )
		{
			sprintf(chOID, "%s", itMRet->first.c_str() );

			objOID.SetSelName(chSelValue);
			objOID.SetOIDValue(chOID);
			objOID.SetOIDType( mapOidType[itMRet->first] );
			...
			if(nResult ==0)
			{
				...
				{
					itMRet->second[resultIt->first] = resultIt->second;

					char strTemp[1024] = {0};
					sprintf( strTemp,"snmp return value:%s.%s:%s\n", 
							 itMRet->second[resultIt->first].m_szOID.c_str(),
							 itMRet->second[resultIt->first].m_szIndex.c_str(),
							 itMRet->second[resultIt->first].m_szValue.c_str() );
		...
		if( bReturn )
		{
			for(i = 0 ; (i < m_nOidCount); i++)
			{
				...
				{	
					resultItem  resultIt = mapOid[chOID].begin();
					if(resultIt != mapOid[chOID].end())
					{
						char szTemp[1024] = {0};
						sprintf( szTemp, "oid=%s,curValue=%s,string=%d,oidHasHistory=%d,hisValue=%s,hisTime=%ld,curTime=%ld", 
								pOidList[i].chOID, resultIt->second.m_szValue.c_str(), pOidList[i].bString, pOidList[i].nHasHistory,
						...
						if(pOidList[i].bString == true )
						{
							char szRetOid[32] = {0};
							sprintf( szRetOid, "oid%d", i+1 );
							mapStringRet[szRetOid] = resultIt->second.m_szValue.c_str();
							continue;
						}
						...
						{   
							...
							expression.AddFields(szOIDName, "value", uSubValue);
							expression.AddFields(szOIDName, "time", lTime);
							expression.AddFields(szOIDName, "avg",  objOID.AvgValue(mapOid[chOID]));
							expression.AddFields(szOIDName, "max", objOID.MaxValue(mapOid[chOID]));
							expression.AddFields(szOIDName, "min", objOID.MinValue(mapOid[chOID]));
							expression.AddFields(szOIDName, "total", objOID.SumValue(mapOid[chOID]));
							expression.AddFields(szOIDName, "indexcount", objOID.IndexCount(mapOid[chOID]));
						}
		...
		for( itMap=mapStringRet.begin(); itMap!=mapStringRet.end(); itMap++ )
		{
			char szTemp[1024] = {0};
			sprintf( szTemp, "oid=%s,value=%s", itMap->first.c_str(), itMap->second.c_str() );
			WriteTxt( szTemp, chMonitorID );
        ...
        if(bReturn)
        {
		    ...
		    {//根据每一个公式计算结果
				...
				{//计算成功 
					...
					WriteTxt( it->first.c_str(), chMonitorID );
					...
					{//如果是第一条计算结果
						...
						else if( ( itMap=mapStringRet.find(it->first)) != mapStringRet.end() )
						{
							sprintf(chReturn, "%s=%s", it->second.c_str(), itMap->second.c_str() );
						}
						else
						{
					...
					{//不是第一条计算结果
						...
						{
							sprintf(chTemp1, "%s=%s", it->second.c_str(), itMap->second.c_str() );
						}
						else
						{
