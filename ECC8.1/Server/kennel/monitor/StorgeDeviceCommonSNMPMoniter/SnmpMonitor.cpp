...
void PrintDebug(const char * szMsg)
{
		...
		printf(szMsg);
		FILE *fp;
		fp=fopen("CommonSNMPdebug.txt","a+");
		fprintf(fp,"%s\n",szMsg);
		fclose(fp);		
	#endif
	return;
...
/////////////////////////////////////////////////////////////////////////////
extern "C" __declspec(dllexport)  int SNMPGetData(const char * strParas, char * szReturn, int& nSize)
{
//	AFX_MANAGE_STATE(AfxGetStaticModuleState());
    int bReturn = TRUE;
	std::string strIP ;
    ...
    snmpOperate.GetResultByOID(szReturn,strOID,nSize);
	PrintDebug(szReturn);
