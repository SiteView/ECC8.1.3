...
public class getActiveDomainAndServers
{
    ...
    public static void main(String args[])
    {
        ...
        {
            System.err.println("FileSplitFlagError=Exception: " + e + "getActive" + "$EndSplitFlag");
            return;
        }
    }
    ...
    public static void GetConnectionPoolInfo(MBeanHome home, String strName)
    {
		...
		//-------------------------------------------------------------------------------------
	  	//增加监测指标 (zouXiao 2008.12.23)
	  	//-------------------------------------------------------------------------------------
		stringbuffer.append("ConnectionsTotalCount=").append(jdbcPool.getConnectionsTotalCount()).append("$");//连接总数

		stringbuffer.append("CurrCapacity=").append(jdbcPool.getCurrCapacity()).append("$");//当前连接池容量
		stringbuffer.append("MaxCapacity=").append(jdbcPool.getMaxCapacity()).append("$");//历史最大连接池容量

		stringbuffer.append("ActiveConnectionsCurrentCount=").append(jdbcPool.getActiveConnectionsCurrentCount()).append("$");//当前活动连接数
		stringbuffer.append("ActiveConnectionsAverageCount=").append(jdbcPool.getActiveConnectionsAverageCount()).append("$");//历史平均活动连接数
		stringbuffer.append("ActiveConnectionsHighCount=").append(jdbcPool.getActiveConnectionsHighCount()).append("$");//历史最大活动连接数

		stringbuffer.append("NumAvailable=").append(jdbcPool.getNumAvailable()).append("$");//当前可利用的连接数
		stringbuffer.append("HighestNumAvailable=").append(jdbcPool.getHighestNumAvailable()).append("$");//历史最大可利用的连接数
		stringbuffer.append("NumUnavailable=").append(jdbcPool.getNumUnavailable()).append("$");//当前不可利用的连接数
		stringbuffer.append("HighestNumUnavailable=").append(jdbcPool.getHighestNumUnavailable()).append("$");//历史最大不可利用的连接数

		stringbuffer.append("WaitingForConnectionCurrentCount=").append(jdbcPool.getWaitingForConnectionCurrentCount()).append("$");//当前等待连接数
		stringbuffer.append("WaitingForConnectionHighCount=").append(jdbcPool.getWaitingForConnectionHighCount()).append("$");//历史最大等待连接数
		stringbuffer.append("WaitSecondsHighCount=").append(jdbcPool.getWaitSecondsHighCount()).append("$");//历史最长等待连接时间

		stringbuffer.append("FailuresToReconnectCount=").append(jdbcPool.getFailuresToReconnectCount()).append("$");//二次连接失败的次数

		stringbuffer.append("ConnectionDelayTime=").append(jdbcPool.getConnectionDelayTime()).append("$");//连接平均耗时

		stringbuffer.append("VersionJDBCDriver=").append(jdbcPool.getVersionJDBCDriver()).append("$");//JDBC驱动版本
