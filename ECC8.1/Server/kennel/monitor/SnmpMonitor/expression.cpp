...
int CSVExpression::isOperator(const string &szOp)
{
    int i = 0, nOpLen = 0;
...
void CSVExpression::AddFields(const string &pcszRow, const string &cszField, unsigned long ulValue)
{
...
//	CSVField svit = svrow->second.find(cszField);


//	FILE *stream;
//	stream = fopen( "fm_Result.txt", "a+" );
//	fprintf(stream,"%s:%d\n",cszField.c_str(),svit->second);			
//	fclose(stream);		
...
int CSVExpression::calculateDouble(const string &szExpr, double &dResult)
{
    int nRet = eval_ok;
    string szRpn("");
    if((nRet = toRPN(szExpr, szRpn)) == eval_ok) // 根据name，解析算法 8 oid7 * oid7.time 1024 * /
    {
		cout << "szRpn" << szRpn <<endl;
...
int CSVExpression::evaluateRPN(const string &rpn, double &result)
{
    ...
    {	
		...
		//if (!st.empty())
		//{
		//	cout << "behind st.top() = " << st.top() << endl;
		//}
    }
    result = st.top();
    st.pop();
...
unsigned long CSVExpression::GetOidValue(const string &chOID)
{
