...
#define __DEFAULT_BUF_SIZE 1024
	...
	bool W3Client::RequestGet(const char *szuri, const char *szref /*=NULL*/){
		...
		{
			m_strErrMsg="additional header failed...";
...
again:
							...
							)
        {
			...
			m_strErrMsg="HttpSendRequest request failed...";
        }
        if (dwError == ERROR_INTERNET_INVALID_CA)
        {
          ...
          InternetSetOption (_hRequest, INTERNET_OPTION_SECURITY_FLAGS,
        ...
        {
            char szMsg[128] = {0};
            sprintf(szMsg,"HttpsMonitor: ----- Request error code is: %u\n" ,dwError);
            OutputDebugString(szMsg);
			m_strErrMsg="Send Request failed...";

            throw "Send Request failed...";
        }
...
std::string W3Client::GetErrorMsg()
{
	return m_strErrMsg;
}
