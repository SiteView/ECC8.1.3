...
class CSnmpOperate  
{
    ...
    {//OID
        ...
        unsigned __int64 ulHisValueList[5];
        unsigned long ulHisTimeList[5];
    }SNMPList;

	typedef struct _SNMPIfOIDList
	{//OID
		_SNMPIfOIDList()
		{
			memset(oid_ifindex, 0, MAX_BUFF_LEN);
			memset(oid_ifdescr, 0, MAX_BUFF_LEN);
			memset(oid_iftype, 0, MAX_BUFF_LEN);
			memset(oid_ifStatus, 0, MAX_BUFF_LEN);
		};

		char oid_ifindex[MAX_BUFF_LEN];//接口索引的OID
		char oid_ifdescr[MAX_BUFF_LEN];//接口描述的OID
		char oid_iftype[MAX_BUFF_LEN];//接口类型的OID
		char oid_ifStatus[MAX_BUFF_LEN];//接口状态的OID
	}SNMPIfOIDList;
