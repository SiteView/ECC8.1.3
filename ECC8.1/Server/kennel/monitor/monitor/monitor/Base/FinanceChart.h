...
class FinanceChart : public MultiChart
{
        ...
        DoubleArray openData, DoubleArray closeData, DoubleArray volData, int extraPoints)
    {
        ...
        deepCopy(m_volData, ArrayMath(volData).div(pow(1000, unitIndex)));
        ...
        int fillColor)
    {
        ...
        return addBand(ArrayMath(movAvg).add(stdDev), ArrayMath(movAvg).sub(stdDev), lineColor,
            fillColor, buffer);
    }
    ...
    {
		char buffer[1024];
        sprintf(buffer, "Bollinger Width (%g, %d)", period, width);
        return addLineIndicator(height, ArrayMath(m_closeData).movStdDev(20).mul(width * 2), color,
            buffer);
    }
    ...
    private: ArrayMath computeTrueLow()
    {
        ...
        for(int i = 0; i < m_lowData.len; ++i) {
            if ((m_volData[i] != Chart::NoValue) && (previousCloseData[i] != Chart::NoValue)) {
                if (m_volData[i] < previousCloseData[i]) {
                    ret[i] = m_lowData[i];
                } else {
