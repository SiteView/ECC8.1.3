					  ...
					  char *proxy_pwd, char *custpath, char *szReturn)
{
	...
	if((ret = cisco_connect1(hSocket, pwd, pwdprompt, prompt,pgd)) < 0)
	{
	...
	if((ret = cisco_connect1(hSocket, enpwd, enpwdprompt, enprompt,pgd)) < 0)
	{
	...
	if((ret = cisco_command1(hSocket, encmd, enprompt,pgd)) < 0)
	{
	...
	index=strBuffer.Find("Invalid source address");
	if(index>=0)
	{
		sprintf(szReturn, "error=%s","Invalid source address- IP address not on any of our up interfaces");//<%IDS_CiscoLog_9%>
		/*telnet_command(hSocket,"exit",pgd);
		Sleep(500);*/
	    delete pgd;
		shutdown(hSocket, 0x02);
	...
	index=strBuffer.Find("Success rate is 0 percent");
	if(index>=0)
	{
		sprintf(szReturn, "packetsGoodPercent=0$roundTripTime=0$");//<%IDS_CiscoLog_9%>
		/*telnet_command(hSocket,"exit",pgd);
		Sleep(500);*/
	    delete pgd;
		shutdown(hSocket, 0x02);
	...
	index=strBuffer.Find("round-trip min/avg/max");
	if(index>=0)
	{
	 int packetIndex=strBuffer.Find("Success rate is");
	 int msIndex=strBuffer.Find("ms");
	 CString tempn=strBuffer.Mid(packetIndex,msIndex-packetIndex);
	 //printf("hhhhhhhhhhhhhhhhhhhhhhhhhhh:%s",tempn);
	 //Success rate is 100 percent (5/5), round-trip min/avg/max = 1/2/4
	 CString spacketsGoodPercent =tempn.Mid(16,3).TrimRight();
	 int packetsGoodPercent=atoi(spacketsGoodPercent);
	 CString tempround=tempn.Mid(tempn.Find("=")+1);
	  printf("hhhhhhhhhhhhhhhhhhhhhhhhhhh:%s",tempround);
	 tempround=tempround.Mid(tempround.Find("/")+1);
	  printf("hhhhhhhhhhhhhhhhhhhhhhhhhhh:%s",tempround);
	 tempround=tempround.Mid(0,tempround.Find("/"));
	 int roundTripTime=atoi(tempround);
	  printf("hhhhhhhhhhhhhhhhhhhhhhhhhhh:%s",tempround);
	 sprintf(szReturn, "packetsGoodPercent=%d$roundTripTime=%d$",packetsGoodPercent,roundTripTime);
	/* telnet_command(hSocket,"exit",pgd);
	 Sleep(500);*/
	 delete pgd;
	 shutdown(hSocket, 0x02);
	...
	sprintf(szReturn, "packetsGoodPercent=0$roundTripTime=0$");
	
//	index = strBuffer.Find("show running-config\r\n", 0);
//	printf("hhhhhhhhhhhhhhhhhhhh:%s  endhhhhhhhhhhhhhhhhhhhh",strBuffer);
//	if(index >= 0)
