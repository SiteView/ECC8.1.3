	...
	if(0 < lenSub && 0 < len)
	{
		...
		for ( LPTSTR lpszReverse = m_pchData + startpos ;
			...
			if (_tcsncmp(lpszSub, lpszReverse, lenSub ) == 0)
				return (int)(lpszReverse - m_pchData);
	}
	return -1;
}
...
CStringEx CStringEx::GetField( LPCTSTR delim, int fieldnum)
{
	...
	while( fieldnum-- >= 0 )
	{
		...
		if( lpsz )
		{
			...
			retlen = static_cast<int>(lpsz - lpszRemainder);
	...
	return Mid( static_cast<int>(lpszret - m_pchData), retlen );
}
...
CStringEx CStringEx::GetDelimitedField( LPCTSTR delimStart, LPCTSTR delimEnd, int fieldnum /*= 0*/)
{
	...
	return Mid( static_cast<int>(lpszRet - m_pchData), static_cast<int>(lpszEnd - lpszRet) );
}
