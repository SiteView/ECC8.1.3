...
bool SubmitUnivData(ForestMap & fmap, const NodeData & inwhat, string & estr)
{
	...
	else if(what.compare("SubmitMonitor")==0)
	{
		...
		string strIsdn = GetValueInForestMap(fmap, "parameter", "isdn", estr2);// 用于判断是否允许添加sip监测器
		...
		if(id.empty())
		{
			...
			if( CanAddMorePoint(parentid, estr, "monitor") )
			{
				...
				if( (
					...
					(!CanAddSipMonitor(parentid, strIsdn, sv_monitortype, estr)) )
				{
					estr+=" 在服务器端创建节点失败。　";
					PutReturnForestMap( fmap,"return","return","false" );
					return false;
		...
		if(ret)
		{
			  ...
			  )// 调用webService进行清零参数设置
			{
				try
				{
					if( SetSingleParameter( fmap,  inwhat, estr ) )
					{
						PutReturnForestMap( fmap,"return","return","true" );
					}
					else
...
bool GetForestData(ForestList & flist, const NodeData & inwhat, string & estr)
{
	string what=GetValueInNodeData(inwhat,"dowhat",estr);
	char strInfo[1024] = {0};
	WriteLog("----------GetForestData Start----------");
	NodeData::const_iterator pNode = inwhat.begin();
	while(pNode != inwhat.end())
	{
		sprintf(strInfo , "%s = %s" , pNode->first.c_str() , pNode->second.c_str());
		WriteLog(strInfo);
		pNode++;
	}
	if(what.empty())
