...
bool GetUnivData(ForestMap & fmap,const NodeData & inwhat, string & estr)
{
	...
	{
		WriteLog("--------DelChildren Start--------");
		ForestMap::const_iterator pMap = fmap.begin();
		NodeData::const_iterator pNode;
		char tempbuf[512] = {0};
		WriteLog("*********ForestMap parameter********");
		while(pMap != fmap.end())
		{			
			sprintf(tempbuf,"[%s]",pMap->first.c_str());
			WriteLog(tempbuf);
			pNode = pMap->second.begin();
			while(pNode != pMap->second.end())
			{
				sprintf(tempbuf , "%s=%s", pNode->first.c_str() , pNode->second.c_str());
				WriteLog(tempbuf);
				pNode++;
			}
			pMap++;
		}
		WriteLog("*********NodeData parameter********");
		pNode = inwhat.begin();
		...
		*/
		...
		try{
			...
			while(  std::getline(input_temp,tempkey)  )
			{
				...
				{
					string strGroupId;
					basic_string<char>::size_type nPos = pid.rfind( '.' );
					if( nPos != basic_string<char>::npos )
					{
						strGroupId = pid.substr( 0, nPos );
					}
					string strNamePlatePWR;
					FindNodeValue(GetEntityMainAttribNode(entityObj) , "_NamePlatePower" , strNamePlatePWR);
					...
					if( !strDeviceType.empty() )
					{
						...
						{
/*--------------------------------------------删除设备后，组剩余功耗增加---------------------------------------------*/
							OBJECT Obj_Group = Cache_GetGroup(strGroupId);
							MAPNODE Group_Node = GetGroupMainAttribNode(Obj_Group);
							string strRestPower , strRackMaxPower , strNewRestPower;
							char strNewPWR[512] = {0};
							sprintf(strNewPWR , "%ld" , atoi(strNamePlatePWR.c_str()) + atoi(strRestPower.c_str()));
							strNewRestPower = strNewPWR;

							FindNodeValue(Group_Node , "TotalRestPower" , strRestPower);
							FindNodeValue(Group_Node , "RackMaxPower" , strRackMaxPower);
							if(atoi(strNewRestPower.c_str()) >= atoi(strRackMaxPower.c_str()))
							{
								AddNodeAttrib(Group_Node , "TotalRestPower" , strRackMaxPower);
							}
							else
							{
								AddNodeAttrib(Group_Node , "TotalRestPower" , strNewRestPower);
							}
	...
	else if(what.compare("TryLogin")==0)
	{
		...
		catch(...)
		{
			...
			FormatMessage ( FORMAT_MESSAGE_ALLOCATE_BUFFER  | 
				...
				NULL,
				errNum, // 此乃错误代码，通常在程序中可由 GetLastError()得之
				LANG_NEUTRAL,
				(LPTSTR) & lpBuffer,
				0 ,
				NULL );
	...
	else if(what.compare("EntityCopy")==0)
	{
		...
		if( autoCreateTable.compare("false")==0 )
			...
			newid = CopyEntity( sourceId, targetParentId, estr, strGroup );
		else
			//newid= EntityCopyAndCreateTable(sourceId, targetParentId);
			newid= CopyEntityAndCreateTable( sourceId, targetParentId, estr ,strGroup);
		...
		else
		{
			...
			PutReturnForestMap( fmap,"return","return","true" );
			PutReturnForestMap( fmap,"return","newid",newid );
			PutReturnForestMap( fmap,"return","groupid",strGroup );
			return true;
		}
	}
	...
	{
		map<string, string>::const_iterator pToWhat = inwhat.begin();
		while(pToWhat != inwhat.end())
		{
			char strWhat[1024] = {0};
			sprintf(strWhat,"%s = %s" , pToWhat->first.c_str() , pToWhat->second.c_str());
			WriteLog(strWhat);
			pToWhat++;
		}
...
bool SubmitUnivData(ForestMap & fmap, const NodeData & inwhat, string & estr)
{
		...
		if(ret)
		{
			if( !strGroupType.empty() && (strGroupType.compare("_dcm") == 0) )
			{
				...
				else
				{
					...
					*/
					//-----------------------------------------------------------------------------------------
					ForestMap activeAlertMap;
					NodeData activeAlertNode;
					string strActiveAlertErr;

					activeAlertNode["parentId"] = id;
					activeAlertNode["ifGroup"] = "0";
					AddActiveAlert(activeAlertMap , activeAlertNode , strActiveAlertErr);
	...
	{
		WriteLog("--------SubmitEntity Start--------");
		char strBuf[512] = {0};
		ForestMap::const_iterator pMap = fmap.begin();
		NodeData::const_iterator pNode;
		WriteLog("********ForestMap Section********");
		while(pMap != fmap.end())
		{
			sprintf(strBuf , "[%s]" , pMap->first.c_str());
			WriteLog(strBuf);
			pNode = pMap->second.begin();
			while(pNode != pMap->second.end())
			{
		...
		*/
		//-----------------------------------------------------------------------------------------
		string strNamePlatePWR = GetValueInForestMap(fmap , "property" , "_NamePlatePower" , estr);		//获取设备铭牌功耗。
		char strTempBuf[512] = {0};
		string parentid= GetValueInNodeData(inwhat,"parentid",estr);

		sprintf(strTempBuf , "GroupID=%s\tNamePlatePower=%s" , parentid.c_str() , strNamePlatePWR.c_str());
		WriteLog(strTempBuf);

		OBJECT Obj_Group = Cache_GetGroup(parentid);
		MAPNODE node = GetGroupMainAttribNode(Obj_Group);
		string strSwitch;
		FindNodeValue(node , "RackSwitch" , strSwitch); 
		sprintf(strTempBuf , "RackSwitch=%s" , strSwitch.c_str());
		WriteLog(strTempBuf);
		if(strSwitch.compare("True") == 0)
		{
			string strRestPower;
			FindNodeValue(node , "TotalRestPower" , strRestPower);
			sprintf(strTempBuf,"TotalRestPower=%s" , strRestPower.c_str());
			WriteLog(strTempBuf);

			sprintf(strTempBuf , "TotalRestPower=%sstrNamePlatePower=%s" , strRestPower.c_str() , strNamePlatePWR.c_str());
			WriteLog(strTempBuf);
			if(atoi(strRestPower.c_str()) < atoi(strNamePlatePWR.c_str()))
			{
				estr += "Not enough power!";
				return false;
			}
		...
		else
		{
			if(!newone)
			{
				...
				if( (strdel.compare("_dcm") == 0) || (strdel.compare("_dcmmanagenode") == 0) || (strdel.compare("_dcmwin") == 0) || (strdel.compare("_dcmunix") == 0))// 若编辑的设备是dcm设备，则编辑前先保留原来属性
				{
					EnumNodeAttrib( GetEntityMainAttribNode(obj), dcmPairList );
				}
			}
		...
		if(ret)
		{
			...
			// 增加时间：2009.03.26
			//--------------------------------------------------------------
			if( sv_devicetype.compare("_dcm") == 0 || sv_devicetype.compare("_dcmwin") == 0 || sv_devicetype.compare("_dcmunix") ==0)
			{
				if( newone )// 创建设备
				{
...
//-----------------------------------------------------------------------------------------
			...
			{
				WriteLog("********sv_devicetype = \"_dcmmanagenode\"********");
				// 恢复svdb中对设备属性的更改，保持与dcm服务器同步
...
/*--------------------------------DCM机架调优策略--------------------------------*/
	else if(what.compare("GetRackMaxPower") == 0)
	{
		bool ret(false);
		try
		{
			...
			string strRackMaxPWR , strLeftPWR;
			ret = GetRackMaxPower(fmap , inwhat , estr , strRackMaxPWR , strLeftPWR);
		}
		catch(...)
		{
			ret = false;
	...
	else if(what.compare("RackDensityTuning") == 0)			//设置机架最大功耗
	{
		bool ret(false);
		try
		{
			ret = RackDensityTuning(fmap , inwhat , estr);
		}
		catch(...)
		{
			ret = false;
	...
	else if(what.compare("GetPowerSavingState") == 0)			//获取DCM组PowerSaving状态
	{
		bool ret(false);
		try
		{
			ret = GetPWRSavStat(fmap , inwhat , estr);
		}
		catch(...)
		{
			ret = false;
	...
	else if(what.compare("PowerSaving") == 0)			//设置机架最大功耗
	{
		bool ret(false);
		try
		{
			ret = PowerSaving(fmap , inwhat , estr);
		}
		catch(...)
		{
			ret = false;
