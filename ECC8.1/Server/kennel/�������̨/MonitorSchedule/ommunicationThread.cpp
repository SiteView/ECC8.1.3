...
void CommunicationThread::run()
{
	...
	//WriteTxt( "============= CommunicationThread::run =============" );
	...
	/*
	char szTemp[1024] = {0};
	sprintf( szTemp, "configtq:%s,scaddr:%s\n", configtq.c_str(), g_ServerHost.c_str() );
	WriteTxt( szTemp );
	*/
...
//		::Sleep(1000*2);
		...
		try{
			...
			/*
			unsigned long lProcessId = GetCurrentProcessId();
			sprintf( szTemp, "------------ProcessId:%ld,Config update:OPT:%s,ID:%s----------------",lProcessId,label.c_str(),id );
			WriteTxt( szTemp );
			*/
