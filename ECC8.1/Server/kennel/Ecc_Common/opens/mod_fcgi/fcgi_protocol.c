...
//char destCookie[1024];
static void add_siteview_wt_session_cgi_var(request_rec *r)
{
	table *e =r->subprocess_env;
	char *srcCookie;
	char *destCookie;
	destCookie =fcgi_buf_new(r->pool,1024);
	srcCookie =ap_table_get(e,"HTTP_COOKIE");
	if(srcCookie!=NULL)
	{
		if(strstr(srcCookie,"svsid=")!=NULL)
			return;
		else  sprintf(destCookie,"%s;svsid=%d",srcCookie,RandIndex());
		
	}else sprintf(destCookie,"svsid=%d",RandIndex());
		
	ap_table_setn(e,"HTTP_COOKIE",destCookie);
	//ap_table_setn(e,"HTTP_COOKIE","svsid=2323");
}
...
int fcgi_protocol_queue_env(request_rec *r, fcgi_request *fr, env_status *env)
{
    ...
    if (env->envp == NULL) {
		...
		{
			ap_add_cgi_vars(r);
			add_siteview_wt_session_cgi_var(r);
		}
