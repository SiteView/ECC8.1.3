...
static int open_connection_to_fs(fcgi_request *fr)
{
	...
	JobState= FCGI_REQUEST_COMPLETE_JOB;		
    ...
    if (fr->dynamic)
    {
...
#endif
        {
            ...
            if (dynamicAutoUpdate) 
            {
				...
				"FCGI: FCGI_SERVER_RESTART_JOB 12000 ");
    ...
    if (socket_path) 
    {
...
#ifdef SITEVIEW
			numChildren=(fr->fs->directive == APP_CLASS_DYNAMIC)
				? dynamicMaxClassProcs
				: fr->fs->numProcesses;				
			proc =fr->fs->procs;

			apcok=ap_table_get(r->subprocess_env,"HTTP_COOKIE");
			
			ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
				"%s request , HTTP_COOKIE  cookie is %s,",fr->fs_path,apcok);
			if(apcok!=NULL){
					__try{
						nSessionid=0;
						tmpSid=strstr(apcok,"svsid=");
						if(tmpSid)
							sscanf(tmpSid, "svsid=%d",&nSessionid);
						else nSessionid=-1;
					}__except(nSessionid=-1)
					{
						nSessionid=-1;
					}
		...
		for (i = 0; i < numChildren; i++, proc++) 
		{
			...
			if((nSessionid==proc->nsessionid)&&(proc->pid>0)&&(proc->bWaitError!=1)   )
				{
					nSelProc=i;
					break;
				}
				if((proc->nsessionid==-1) &&(proc->pid>0))
				{
					nNewProc=i;
				}
			...
			ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
				"%s request , HTTP_COOKIE  sessionid is %d,nSel=%d,nNew=%d ",
				fr->fs_path, nSessionid,nSelProc,nNewProc	);
		...
		if(nSelProc==-1)
		{
			if(nNewProc==-1)
			{
				...
				fr->fs->procs[nNewProc].nsessionid= nSessionid;
				hCgiEvent= fr->fs->procs[nNewProc].hCgiEvent;
				hModEvent=  fr->fs->procs[nNewProc].hModEvent;
				outpid = fr->fs->procs[nNewProc].pid;
				nSelProc =nNewProc;
				fr->nRunProc = nNewProc;



				ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
					" %s EventName is %s ,pid is %d ",
					fr->fs_path,
					fr->fs->procs[nNewProc].pidEventName,
					fr->fs->procs[nNewProc].pid);

			}
		}else
		{

			//SetEvent(fr->fs->procs[nSelProc].hCgiEvent);
			//sprintf(needSetEventName,"%s",fr->fs->procs[nSelProc].hCgiEvent);
			hCgiEvent= fr->fs->procs[nSelProc].hCgiEvent;
			hModEvent=  fr->fs->procs[nSelProc].hModEvent;

			ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
				"FCGI: request ,EventName is %s,pid is %d  ",
				fr->fs->procs[nSelProc].pidEventName,
				fr->fs->procs[nSelProc].pid);
			outpid = fr->fs->procs[nSelProc].pid;
			fr->nRunProc = nSelProc;

		}

		SetEvent(hCgiEvent);
		Sleep(100);
		ret= WaitForSingleObject(hCgiEvent,0 );

		sprintf(needSetEventName,"  rewait event  %d \n",  outpid);

		for(iReady=0;iReady<20;iReady++)
		{
			if(ret==WAIT_TIMEOUT)
				break;
			Sleep(500);
			...
			//OutputDebugString( needSetEventName );

		}

		sprintf(needSetEventName,"  wait event  %d  success \n",  outpid);
		...
		ready = WaitNamedPipe(socket_path, 10000);
		...
		if (ready)
		{
			...
			if (fr->fd != (SOCKET) INVALID_HANDLE_VALUE) 
			{
				...
				//ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
				//	"got npipe connect: %s", fr->fs_path);
				return FCGI_OK;
			}
		...
		{
			ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
		"FCGI: WaitNamedPipe Error ,EventName is %s,interval is %d  ",
		fr->fs->procs[nSelProc].pidEventName,
		interval);
			if(fr->nRunProc>=0)
				fr->fs->procs[fr->nRunProc].bWaitError =1;
...
SITEVIEW_PID:
			...
			{
				ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
					"-------------aaaaa--------FCGI_SERVER_START_JOB--------------- %s",fr->fs_path);
				send_to_pm(FCGI_REQUEST_TIMEOUT_JOB, fr->fs_path, fr->user, fr->group, 0, 0);
    ...
    if (fr->dynamic && errno == ECONNREFUSED) {
		...
		ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
				"FCGI: FCGI_SERVER_RESTART_JOB 12002 ");
    ...
    if (fr->dynamic) {
        do {
            ...
            /* select() timed out */
			ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
				"FCGI: FCGI_SERVER_RESTART_JOB 12003 ");
...
SERVER_SEND:
        ...
        case STATE_SERVER_SEND:
            ...
            if (! is_connected) 
            {
                ...
                {
					ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
					"---------------------open_connection_to_fs error--------------- %s",fr->fs_path);
...
//						"ERROR_NO_DATA");
	...
	//	ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
	//		"BufferLength(fr->clientOutputBuffer) is %d,",
	//		BufferLength(fr->serverInputBuffer) );
        ...
        else if (dynamic_first_recv)
        {
            ...
            else 
            {
				...
				ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
				"FCGI: FCGI_SERVER_RESTART_JOB 12004 --%d",delay);
				state = STATE_ERROR;
				break;
        ...
        if (recv_pending)
        {
			...
			msec_timeout =max(msec_timeout,180000);
            ...
            if (rv == WAIT_TIMEOUT)
            {
                ...
                else if (dynamic_first_recv)
                {
					...
					ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
						"FCGI:  FCGI_REQUEST_TIMEOUT_JOB dynamic_first_recv  %d",msec_timeout);
					fr->fs->procs[fr->nRunProc].bWaitError= 1;
	...
	//	ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
	//		"before fcgi_protocol_dequeue is %d,",BufferLength(fr->serverInputBuffer) );
     ...
     //   ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
	//		"after fcgi_protocol_dequeue is %d,",BufferLength(fr->serverInputBuffer) );
	...
	//	ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
	//		"before process_headers is %d,",BufferLength(fr->clientOutputBuffer) );
...
SERVER_SEND:
        ...
        else if (dynamic_first_recv)
        {
            ...
            else 
            {
                ...
                if (idle_time.tv_sec > idle_timeout) 
                {
                    //send_to_pm(FCGI_REQUEST_TIMEOUT_JOB, fr->fs_path, fr->user, fr->group, 0, 0);
                    ap_log_rerror(FCGI_LOG_ERR_NOERRNO, r, "FastCGI: comm "
						"with (dynamic) server \"%s\" aborted: (first read) fr->nRunProc is %d  "
                        "idle timeout (%d sec)", fr->fs_path, idle_timeout ,fr->nRunProc );
					fr->fs->procs[fr->nRunProc].bWaitError= 1;
	    ...
	    FCGIDBG5("qwait=%ld.%06ld delay=%d first_recv=%d", qwait.tv_sec, qwait.tv_usec, delay, dynamic_first_recv);
            ...
            else 
            {
                /* Killed time somewhere.. client read? */
				ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
				"FCGI: FCGI_SERVER_RESTART_JOB 12006 ");
        ...
        if (select_status == 0) 
        {
            ...
            else if (dynamic_first_recv) 
            {
				...
				ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
				"FCGI: FCGI_SERVER_RESTART_JOB 12007 ");
...
//	ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
//		"close_connection_to_fs %d,",BufferLength(fr->clientOutputBuffer) );
    ...
    {
//		ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
//			"sink_client_data %d,",BufferLength(fr->clientInputBuffer) );
        sink_client_data(fr);
//		ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
//			"sink_client_data %d,",BufferLength(fr->clientInputBuffer) );
