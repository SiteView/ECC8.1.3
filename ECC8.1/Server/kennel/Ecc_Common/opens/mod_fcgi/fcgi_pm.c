...
#ifdef SITEVIEW
	//process->nstartjob=0;
	process->nsessionid=-1;
	strcpy(process->pidEventName,"");
	process->bWaitError=0;
	if(process->hCgiEvent)  CloseHandle(process->hCgiEvent);
	if(process->hModEvent)  CloseHandle(process->hModEvent);
...
//	char pidEventName[256];
	...
	//		"FCGI: createprocess ,namepipe is %s ",fs->socket_path		);
	...
	//ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
	//	"FCGI: befor cgi_build_command,namepipe is %s ",fs->socket_path		);
    ...
    if (apr_procattr_child_in_set(procattr, file, NULL))
        goto CLEANUP; 
	//ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
	//	"FCGI: befor apr_proc_create,namepipe is %s ",fs->socket_path		);
	
    if (apr_proc_create(&proc, command, argv, fs->envp, procattr, tp))
        goto CLEANUP;
	//ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
	//	"FCGI: after apr_proc_create,namepipe is %s ",fs->socket_path		);
	

	//Sleep(200);
    process->handle = proc.hproc;
	
	sprintf(process->pidEventName,"SiteView-Cgi-Event-%d",proc.pid);
	
	sprintf(modEventName,"ModFcgi-Event-%d",GetCurrentProcessId());	

	
	process->hCgiEvent=CreateEvent(NULL,TRUE,FALSE,process->pidEventName);
	process->hModEvent=CreateEvent(NULL,TRUE,FALSE,modEventName);
	process->bWaitError =0;
	
	//process->nstartjob=1;
	process->nsessionid=-1;
	
	//SetEvent(process->hCgiEvent);
CLEANUP:
	...
	ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
		"FCGI: createprocess ,threadid is %d",
		process.threadpid);
                      ...
                      &si, &pi)) 
    {
		...
		process->treadpid =pi.dwThreadId;
...
static void dynamic_kill_idle_fs_procs(void)
{
    ...
    for (s = fcgi_servers;  s != NULL; s = s->next) 
    {
		...
		//for (i = 0; i < dynamicMaxClassProcs; ++i)
  //      {
		//	if ((s->procs[i].state == FCGI_RUNNING_STATE)&&(s->procs[i].bWaitError==1) )
		//		fcgi_kill(&s->procs[i], SIGTERM);
		//	
  //      }
            ...
            for (i=0; i < numChildren; i++) {
                if (s->procs[i].handle != INVALID_HANDLE_VALUE) 
                {
                    ...
                    if (dwRet != WAIT_TIMEOUT && dwRet != WAIT_FAILED) {
						...
						s->procs[i].state=FCGI_READY_STATE;
						//s->procs[i].pid=-1;
						s->procs[i].hCgiEvent=NULL;
						s->procs[i].hModEvent=NULL;
						s->procs[i].bWaitError= 0;
						
						//s->procs[i].nstartjob=0;
						
						s->procs[i].nsessionid=-1;
						strcpy(s->procs[i].pidEventName,"");	
        ...
        for (s = fcgi_servers; s != NULL; s = s->next) 
        {
            ...
            for (i = 0; i < numChildren; ++i) 
            {
                ...
                if (s->procs[i].pid <= 0 && s->procs[i].state == FCGI_START_STATE)
                {
					...
					//ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
						//"FCGI: createprocess  spawn_fs_process");
//					ap_log_error(FCGI_LOG_NOTICE_NOERRNO, fcgi_apache_main_server,
//						"FastCGI:  (failed number is )");
...
//                    ap_log_error(FCGI_LOG_NOTICE_NOERRNO, fcgi_apache_main_server,
//						"FastCGI:  (failed number is %d)", s->numFailures );
...
#endif
						ap_log_error(FCGI_LOG_WARN_NOERRNO, fcgi_apache_main_server,
							"FCGI: createprocess  spawn_fs_process,%d",numChildren);
						
                        s->procs[i].pid = spawn_fs_process(s, &s->procs[i]);
						//ap_log_error(FCGI_LOG_CRIT, fcgi_apache_main_server,
                          //      "FastCGI: pid is %d",s->procs[i].pid );
