...
//	printf("ppppppppppppppppppppp1:%d\n",::GetTickCount()-bbe);
	switch(pending)
	{
	case pendingInput:
...
//	printf("pppppppppppppppppppp2:%d\n",::GetTickCount()-bbe);
#ifdef	WIN32
//	Thread::exitCancel(cancel);
#endif
//	printf("pppppppppppppppppppp3:%d\n",::GetTickCount()-bbe);
	if(status < 1)
		return false;
	if(FD_ISSET(so, &grp))
		return true;
...
CCXX_EXPORT(ostream&) operator<<(ostream &os, const IPV4Address &ia)
{
