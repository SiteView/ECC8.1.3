...
char snmp_cpp_version[]="#(@) SNMP++ $Id: uxsnmp.cpp,v 1.14 2004/11/06 12:12:08 katz Exp $";
...
//  printf("securityLevel :%d  engineer func \r\n",pdu.get_security_level());
...
int receive_snmpdiscovery_response(int sock, Snmp &snmp_session,
                          Pdu &pdu, UdpAddress &fromaddress,
			  bool process_msg = true)
{
	//printf("in response \r\n");
  ...
  if (process_msg == false)
    return SNMP_CLASS_SUCCESS;   // return success
 // printf("process_msg  ok \r\n"	);

  SnmpMessage snmpmsg;
  if ( snmpmsg.load( receive_buffer, receive_buffer_len) != SNMP_CLASS_SUCCESS)
    return SNMP_CLASS_ERROR;

//  printf("snmpmsg.load  ok \r\n"	);
...
//  printf("ready parser \r\n"	);
  ...
  if (snmpmsg.is_v3_message() == TRUE)
  {
    int returncode = snmpmsg.unloadv3(pdu, version, engine_id,
	 ...
	 printf("receive_snmp_response: engine_id (%s), security_name (%s), security_model (%i), security_level (%i)",
                engine_id.get_printable(), security_name.get_printable(),
                security_model, pdu.get_security_level());
    ...
    if (returncode != SNMP_CLASS_SUCCESS)
	{
		printf("snmpmsg.unloadv3 failed \r\n"	);
      return returncode;
	}
			      ...
			      const OctetStr *community)
{
  ...
  do
  {
    ...
    if ((nfound > 0) && (FD_ISSET(socket, &readfds)))
    {
	 ...
	 // if (receive_snmp_response(socket, *this, dummy_pdu,
	  //			from, false  /* process_msg */)
	   if (receive_snmpdiscovery_response(socket, *this, dummy_pdu,
	  			from, true  /* process_msg */)
	  == SNMP_CLASS_SUCCESS)
      {
			printf("----- enegeerid :%s \r\n"	,dummy_pdu.get_context_engine_id().get_printable());
			addresses += from;
...
// Send a SNMP Broadcast message.
int Snmp::unicast_discovery( OctetStr& engine_id,
			      const int timeout_sec,
			      const UdpAddress &addr,
			      const snmp_version version,
			      const OctetStr *community)
  ...
  do
  {
    ...
    if ((nfound > 0) && (FD_ISSET(socket, &readfds)))
    {
	  ...
	  if (receive_snmpdiscovery_response(socket, *this, dummy_pdu,
	  	  ...
	  	  == SNMP_CLASS_SUCCESS)
      {
		    engine_id=dummy_pdu.get_context_engine_id();
			break;
