...
std::vector<char> mailer::makesmtpmessage() const {
   ...
   if(ptm) {
      ...
      if(ptm->tm_mday < 10) // add a trailing zero if single digit
         str << "0";
      str << ptm->tm_mday << " ";
      switch(ptm->tm_mon) {
      case 0:
      ...
      case 11:
         ...
         break;
      default:
         str << "Jan "; // be safe, pretend the beginning of the year
      }
      ...
      // Hope this is correct, it works for me, however ( EST time ).
      tzset();
#ifdef WIN32
