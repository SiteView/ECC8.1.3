...
//	return;
	...
	sprintf(szProgramName , "scasvapi_%s.log" , datebuf);
...
bool PutForestMapIntoMapnode(const ForestMap & fmap, string section, MAPNODE ma, bool clearAtFirst=false)
{
	...
	if(fit!=fmap.end())
	{
		for(NodeData::const_iterator nit=fit->second.begin(); nit!=fit->second.end(); ++nit)
		{
			...
			if(nit->first.compare("line") == 0)
			{
				AddNodeAttrib(ma , nit->first , "1,");
			}
		}
	}
	return true;
...
bool GetUnivData(ForestMap & fmap,const NodeData & inwhat, string & estr)
{
	...
	//---------------------------------------------------------------------
	// DeleteIniFileSection: 增加删除所有section功能
	// 修改人：邹晓
	// 修改时间：2009.06.08
	//---------------------------------------------------------------------
	else if(what.compare("DeleteIniFileSection")==0)
	{
		...
		//string strDelAll = GetValueInNodeData(inwhat, "delAll", estr);
		string strDelAll = "false";

		string strDelFile = GetValueInNodeData( inwhat, "delFile", estr );

		if( strDelAll.compare("true") == 0 )// 删除所有section功能
		{
			std::list<string> listSection;
			std::list<string>::iterator it;
			GetIniFileSections( listSection, filename );
		...
		try{
			...
			while(  std::getline(input_temp,tempkey)  )
			{
				...
				{
					if( strDelFile.compare("true") == 0 )// 清空对应section的文件
					{
						string strSectionFile = tempkey + ".ini";
						std::list<string> listSection;
						std::list<string>::iterator it;
						GetIniFileSections( listSection, strSectionFile );

						for( it=listSection.begin(); it!=listSection.end(); it++ )
						{
							DeleteIniFileSection( *it,strSectionFile,"localhost",user);
						}
					}
...
sprintf( szTemp, "%s%s=%s\n", szTemp, nit->first.c_str(), nit->second.c_str() );
	...
	else if(what.compare("EntityCopy")==0)
	{
		...
		// 判断是否是华为设备		
		bool bISHW(false);
		OBJECT	objEntity = Cache_GetEntity( sourceId );
		if( objEntity != INVALID_VALUE )
		{
			...
			if( node != INVALID_VALUE )
			{
				...
				if( !strHWID.empty() )// 是华为设备
				{
					bISHW = true;

					string strSE = sourceId.substr( 0, 2 );
					if( strSE.compare("1.") == 0 )
					{
						AddNodeAttrib( node, "sv_devicetype", "_Sip" );
						AddNodeAttrib( node, "sv_network", "true" );
						AddNodeAttrib( node, "sv_dependscondition", "3" );
...
*/
	...
	// 下列分支用于华为设备
	//----------------------------------------------------------
	else if(what.compare("valueChange")==0)
	{
		bool ret(false);
		try{
			ret= ValueChange(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
	...
	else if(what.compare("boot")==0 || what.compare("Periodic")==0)
	{
		bool ret(false);
		try{
			ret= Boot(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
	...
	else if(what.compare("getParameter")==0)
	{
		bool ret(false);
		try{
			ret= GetParameter(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
	...
	else if(what.compare("setParameter")==0)
	{
		bool ret(false);
		try{
			ret= SetParameter(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
	...
	else if(what.compare("getAlertID")==0)
	{
		bool ret(false);
		try{
			ret= GetAlertID(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
	...
	else if(what.compare("getSipIsdn")==0)
	{
		bool ret(false);
		try{
			ret= GetSipIsdn(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
			estr+= "  Exception in getSipIsdn;   ";
		}
		if(ret)
			PutReturnForestMap( fmap,"return","return","true" );
		else
...
/*--------获取监测器所属性的设备类型 Start--------*/
		...
		entityObj = Cache_GetEntity(parentid);
...
/*--------获取监测器所属性的设备类型 End--------*/
		...
		{
			strIsdn = GetValueInForestMap(fmap, "parameter", "isdn", estr2);// 用于判断是否允许添加sip(TG)监测器
		}
		else if(strDevType.compare("_SipAG") == 0)
		{
			strIsdn = GetValueInForestMap(fmap, "parameter", "line", estr2);// 用于判断是否允许添加sip(AG)监测器
			strIsdn += ",";
		}
		...
		if(id.empty())
		{
			...
			{
				// 判断是否可以添加sip监测器
				int nMonitorType = atoi(sv_monitortype.c_str());
				if( (
					(nMonitorType == 1400) ||
					(nMonitorType == 1401) ||
					(nMonitorType == 1402) ||
			...
			/*--------下面的是AG设备下的监测器--------*/
					...
					(nMonitorType == 1414) ||
					(nMonitorType == 1415)	
					) &&( !CanAddSipMonitor(parentid , strIsdn , sv_monitortype , strDevType , estr)) )
				{
					estr+=" 在服务器端创建节点失败。　";
					PutReturnForestMap( fmap,"return","return","false" );
					WriteLog(estr.c_str());
		...
		bool bResetChange(false);// 清零参数是否发生变化
		...
		{
			// 确认清零参数是否发生变化
			// 当提交的参数中"resetStatistics"为空，或者提交的"resetStatistics"值与监测器中原来的不同时。
			string strResetStatistics;
			FindNodeValue( GetEntityMainAttribNode(obj), "resetStatistics", strResetStatistics );
			string strSubResetParam = GetValueInForestMap(fmap, "parameter", "resetStatistics", estr2);
			if(	(!strSubResetParam.empty()) &&
				(strSubResetParam.compare(strResetStatistics) != 0) )
			{
				bResetChange = true;
			}
			ForestMap & fmap1= const_cast< ForestMap & >( fmap );
		...
		if(ret)
		{
			...
			/*--------下面的是AG设备下的监测器--------*/
					...
					(nMonitorType == 1414) ||
					(nMonitorType == 1415)				) 
			  )// 调用webService进行清零参数设置
			{
				try
				{
					if( SetSingleParameter( fmap,  inwhat, strDevType , estr ) )
					{
						PutReturnForestMap( fmap,"return","return","true" );
					}
					else
				...
				catch( ... )
				{
					...
					estr+= "  Exception in SetParameter;   ";
					estr += lpBuffer;
					::LocalFree(lpBuffer);
					PutReturnForestMap( fmap,"return","return","false" );
				}				
	...
	//----------------------------------------------------------
	// 增加内容：delIniFile:用于删除ini文件的接口"delIniFile"
	//           getParameter:用于获取华为设备cpe信息
	//           setParameter:用于设置华为设备cpe信息
	//           valueChange: 用于华为设备
	//           boot:        用于华为设备
	//
	// 增加人  ：邹晓
	// 增加时间：2009.05.13
	//----------------------------------------------------------
	else if(what.compare("delIniFile")==0)
	{
		/*
		bool ret(false);
		try{
		...
		string strIniFile = GetValueInNodeData( inwhat, "filename",	estr );
		if( strIniFile.empty() )
		{
			estr += "缺少文件名，删除文件失败！";
			return false;
	...
	else if(what.compare("MassMoveEntity")==0)
	{
		bool bRet(false);

		try
		{
			bRet = MassMoveEntity( fmap, inwhat, estr );
		}
		catch(...)
		{
			estr+= "  Exception in MassEntityCopy;   ";
			PutReturnForestMap( fmap,"return","return","false" );
			return false;
		}
	...
	else if(what.compare("saveConditionToMTP")==0)
	{
		bool ret(false);
		try{
			ret= SaveConditionToMTP(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
			estr+= "  Exception in saveConditionToMTP;   ";
		}
		if(ret)
			PutReturnForestMap( fmap,"return","return","true" );
		else
	...
	else if(what.compare("massAddMonitor")==0)
	{
		bool ret(false);
		try{
			ret= MassAddMonitor(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
			estr+= "  Exception in massAddMonitor;   ";
		}
		/*
		if(ret)
			PutReturnForestMap( fmap,"return","return","true" );
	...
	else if(what.compare("CpeTransferComplete") == 0)
	{
		bool ret(false);
		try
		{
			ret= CpeTransferComplete(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
			estr+= "  Exception in CpeTransferComplete;   ";
	...
	else if(what.compare("SipDownLoad") == 0)
	{
		bool ret(false);
		try
		{
			ret= SipDownLoad(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
			estr+= "  Exception in SipDownLoad;   ";
		}
		if(ret)
			PutReturnForestMap( fmap,"return","return","true" );
		else
	...
	else if(what.compare("QueryDownloadStatus") == 0)
	{
		bool ret(false);
		try
		{
			ret= QueryDownloadStatus(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
	...
	else if(what.compare("SipReboot") == 0)
	{
		bool ret(false);
		try
		{
			ret= SipReboot(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
	...
	/*
	else if(what.compare("massGetSipIsdn")==0)
	{
		bool ret(false);
		try{
			ret= MassGetSipIsdn(fmap, inwhat, estr);
		}
		catch(...)
		{
			ret=false;
			estr+= "  Exception in massGetSipIsdn;   ";
		}
		if(ret)
			PutReturnForestMap( fmap,"return","return","true" );
		else
