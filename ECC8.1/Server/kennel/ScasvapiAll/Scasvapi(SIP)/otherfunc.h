...
bool CpeTransferComplete(ForestMap &fmap , const NodeData &inwhat , string &estr);		//下载完成后WebService返回下载状态
bool SipDownLoad(ForestMap &fmap , const NodeData &inwhat , string &estr);				//客户端提交下载请求的接口。
bool QueryDownloadStatus(ForestMap &fmap ,const NodeData &inwhat , string &estr);			//客户端查询下载状态的接口
bool SipReboot(ForestMap &fmap , const NodeData &inwhat , string &estr);						//客户端提交Reboot命令到WebService的接口
#endif
