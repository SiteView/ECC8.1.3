...
typedef BOOL (SipDL) (const char * strParas , char * szReturn, int &nSize);
...
bool QueryRecordsByTime(ForestList & fmap,  const NodeData & inwhat, string & estr)
{
	...
	if ( rds==INVALID_VALUE )  
	{
		estr+="  rds==INVALID_VALUE  ";
		//PutReturnForestMap( fmap,"return","return","false" );
		WriteLog("QueryRecordsByTime failed!");
		WriteLog(estr.c_str());
		return false;
	}
	//PutReturnForestMap( fmap,"return","id",id );
...
bool QueryRecordsByCount(ForestList & fmap,  const NodeData & inwhat, string & estr)
{
	...
	if ( rds==INVALID_VALUE )  
	{
		estr+="  rds==INVALID_VALUE  ";
		//PutReturnForestMap( fmap,"return","return","false" );
		WriteLog("QueryRecordsByCount failed!");
		WriteLog(estr.c_str());
		return false;
	}
	//PutReturnForestMap( fmap,"return","id",id );
...
bool CanAddMorePoint2(const string & entityid, string & estr, string action, int monitorAddNum=1)
{
	...
	*/
	...
	if(pointsum==0)
	{
		...
		WriteLog("CanAddMorePoint2 failed!");
		WriteLog(estr.c_str());
		return false;
	}
...
bool AddManyMonitor(ForestMap & fmap,  const NodeData & inwhat, string & estr)
{
	...
	{
		estr+=" 没有传入 monitor;  ";
		WriteLog("AddManyMonitor failed!");
		WriteLog(estr.c_str());
		return false;
	}
		...
		case WAIT_OBJECT_0:
			try
			{
				...
				for( PAIRLIST::iterator it=listSVSE.begin(); it!=listSVSE.end(); it++ )
				{
					...
					PAIRLIST listHWID;
					bool bEntity(false);
					bEntity = GetAllEntitysInfo( listHWID, "hwID", svseID );
					if(!bEntity)
					{
						estr += "Get all entity information failed!";						
						ReleaseMutex(hMutex);
						WriteLog("ValueChange failed!");
						WriteLog(estr.c_str());
						return false;
					...
					{
						string strTempHwId = itHWID->value;
						if(strTempHwId.compare(strHWID.c_str()) == 0)
...
bool strTrim(string strSource , string &strObject)
{
	char * strReceive = NULL;
	try
	{
		strReceive = new char[(int)strSource.size()];
		char steps[] = " ";
		char * strToken = NULL;
		sprintf(strReceive , "%s" , strSource.c_str());
		strToken = strtok(strReceive , steps);
		while(strToken !=NULL)
		{
			strObject += strToken;
			strToken = strtok(NULL , steps);
		}
	}
	catch(...)
	{
...
{
	char			szTemp[1024*10] = {0};		//临时变量，主要用于辅助打印日志函数，将各类数据格式转化成字符串，以便于WriteLog调用
	list<string>	pfList;						//profile链表，Boot时先从注册表里面获取各社备里面的proFile类型，判断是AG还是TG的设备，做不同的处理
	list<string>::iterator pPFList;
	string			strDevType = "";			//设备类型，目前可能为AG或者TG。2009-12-14新增DLink设备
	bool			bType = false;				//如果设备类型为AG，则为false，如果设备类型为TG，则为true;
	int				nType = 0;					//设备类型：1为TG，2为AG，3为DLink，
	string			strTempProfile;				//临时profile变量，将传入的profile及配置文件里面的profile去掉空格后做比较用
	...
	WriteIniFileString(strManufacturer , strProductClass , strProductClass , "ManufacturerInfo.ini");
	strTrim(strProFile , strTempProfile);

/*2009-12-14新增Dlink设备，2009-12-18新增Alpha设备*/
	SvIniFile iniFile;
	SvIniFile::iterator pIni;
	NodeData::iterator pNodeBoot;

	GetSvIniFileBySections(iniFile , "sipboot.ini" , "DeviceType");
	pNodeBoot = iniFile.begin()->second.begin();
	while(pNodeBoot != iniFile.begin()->second.end())
	{
		string strTmpInputProfile;
		strTrim(pNodeBoot->first , strTmpInputProfile);
		if(strTmpInputProfile.compare(strTempProfile.c_str()) == 0)
//		if(pNodeBoot->second.compare(strTempProfile.c_str()) == 0)
		{
			strDevType = pNodeBoot->second;
			break;
		}
		pNodeBoot++;
	}
...
// 获取SVSE数目
			...
			for( PAIRLIST::iterator it=listSVSE.begin(); it!=listSVSE.end(); it++ )
			{
				...
				while(1)
				{
					...
					if(nTimesToGetAllEntity >= 5)
					{
						estr += "Get all entity information failed!";
						ReleaseMutex(hMutex);
						return false;
					}
					nTimesToGetAllEntity++;
					Sleep(3000);
				}
				...
				for( PAIRLIST::iterator itHWID=listHWID.begin(); itHWID!=listHWID.end(); itHWID++ )
				{
					string strTempID = itHWID->value;
					if(strTempID.compare(strHWID.c_str()) == 0)
//					if( itHWID->value.compare(strHWID) == 0 )
					{
						strID = itHWID->name;
						bFind = true;
...
FIND:
			...
			else// 没有找到设备，将设备加入新增加设备节点
			{
				...
				if( objEntity != INVALID_VALUE )
				{
					...
					if( node != INVALID_VALUE )
					{
/*--------如果是TG设备，则设备类型为_Sip，如果是TG设备，则设备类型为_SipAG------- 2009-12-14做新的判断处理
						if(bType)
						{
							AddNodeAttrib( node, "sv_devicetype", "_Sip" );
						}
						...
						{
							AddNodeAttrib( node, "sv_devicetype", "_SipAG" );
						}
*/         
						if(strDevType.compare("DLink") == 0)
						{
							AddNodeAttrib(node , "sv_devicetype" , "_DLink");
						}
						else if(strDevType.compare("AG") == 0)
						{
							AddNodeAttrib(node , "sv_devicetype" , "_SipAG");			//2009-12-14 add
						}
						else if(strDevType.compare("TG") == 0)
						{
							AddNodeAttrib(node , "sv_devicetype" , "_Sip");			//2009-12-14 add
						}
						else if(strDevType.compare("Alpha") == 0)					//2009-12-18 add
						{
							AddNodeAttrib(node , "sv_devicetype" , "_Alpha");
						}
...
/*--------如果是AG设备，就不查找isdn数目，仅TG设备有isdn数目--------*/
//					if(bType)
					if(strDevType.compare("TG") == 0)
					{
						// 获取isdn数目
						// 设置参数
...
FIND:
	...
	*/
	...
	string strUserName;			//2009-12-14新增参数
	string strPassWord;			//2009-12-14新增参数

	string strMachinePort;		//2009-12-16新增参数
	...
	if( objEntity != INVALID_VALUE )
	{
		...
		if( node != INVALID_VALUE )
		{
			...
			FindNodeValue(node , "UserName" , strUserName);
			FindNodeValue(node , "PassWord" , strPassWord);

			FindNodeValue(node , "_MachinePort" , strMachinePort);
			if(!strMachinePort.empty())
			{
				strIP += ":" + strMachinePort;
			}
		}
	}
//对密码进行解密后再传
	char strBuf[128] = {0};
	Des des;
	des.Decrypt(strPassWord.c_str() , strBuf);
	strPassWord.clear();
	strPassWord = strBuf;
		...
		if( pFun != NULL )
		{
			try
			{
				...
				*/
				...
				mapParam["UserName"] = strUserName;
				mapParam["PassWord"] = strPassWord;
...
bool SetParameter(ForestMap& fmap,  const NodeData& inwhat, string& estr)
{
	...
	if( objEntity != INVALID_VALUE )
	{
		...
		if( node != INVALID_VALUE )
		{
			...
			FindNodeValue( node, "serverIP", strServerIP );
			FindNodeValue( node, "serverPort", strServerPort );
			FindNodeValue(node , "UserName" , strUserName);		//2009-12-15新增参数
			FindNodeValue(node , "PassWord" , strPassWord);		//2009-12-15新增参数
			FindNodeValue(node , "_MachinePort" , strMachinePort);	//2009-12-16新增
			if(!strMachinePort.empty())
			{
				strIP += ":" + strMachinePort;
			}
...
bool GetDynParams( map<string,string> mapParams, list<string>& listDynParams, string& strToken, string& strErr )
{
	...
	{
		strErr += "动态参数名为空， ";
		WriteLog("GetDynParams failed!");
		WriteLog(strErr.c_str());
		return false;
	}
...
bool SetSingleParameter(ForestMap& fmap,  const NodeData& inwhat , string strDevType , string& estr)
{
	string strIsReset;
	...
	if( itMap != fmap.end() )
	{
		...
		if( strIsReset.empty() )
		{
			...
			WriteLog("SetSingleParameter failed!");
			WriteLog(estr.c_str());
			return false;
		}		
...
// 获取动态参数链表，对TG设备类型有效
		...
		else if(strDevType.compare("_SipAG") == 0)
		{
			map<string,string> mapParams = itMap->second;
			list<string> listDynParams;
			string strToken;
...
bool SaveConditionToMTP(ForestMap& fmap,  const NodeData& inwhat, string& estr)
{
	...
	{
		estr += "缺少监测器类型！";
		WriteLog("SaveConditionToMTP failed!");
		WriteLog(estr.c_str());
		return false;
	}
...
bool CanAddSipMonitor( string strEntityId, string strIsdn, string strMonitorType , string strDevType , string& estr)
{
	string strHWID;
	string strExistIsdn;
	char strTestMsg[128] = {0};

	if( strEntityId.empty() )
	{
		estr +=" 传入的设备Id为空 ";
		WriteLog("CanAddSipMonitor failed!");
		WriteLog(estr.c_str());
		return false;
	}

	sprintf(strTestMsg , "strIsdn=%s" , strIsdn.c_str());
	WriteLog(strTestMsg);
	try
	{
						...
						if( GetSubMonitorsIDByEntity( entityObj, monitorList ) )
						{
							for( itList=monitorList.begin(); itList!=monitorList.end(); itList++ )
							{
								...
								if( monitorObj != INVALID_VALUE )
								{
									...
									if(strDevType.compare("_Sip") == 0)		//如果是TG设备，还需要检查isdn是否相同
									{
									...
									else if(strDevType.compare("_SipAG") == 0)
									{
										...
										if( node != INVALID_VALUE )
										{
											FindNodeValue( node, "line", strOtherIsdn );
											char otherIsdn[128] = {0};
											sprintf(otherIsdn , "strOtherIsdn=%s" , strOtherIsdn.c_str());
											WriteLog(otherIsdn);
											string isdnList;
											list<string>::iterator itTmp;
											for( itTmp=listIsdn.begin(); itTmp!=listIsdn.end(); itTmp++ )
											...
											sprintf(otherIsdn , "isdnList=%s" , isdnList.c_str());
											WriteLog(otherIsdn);
										}
...
void CreateMonitorLabelTree( ForestMap& monitorTree )
{
	...
	catch( ... )
	{
		...
		char strMsg[128] = {0};
		sprintf(strMsg , "Exception occured in CreateMonitorLabelTree! Error code:%d , Error msg: %s" , dwError , lpBuffer);
		WriteLog(strMsg);
		::LocalFree(lpBuffer);
	}
...
void CreateAGMonitorLabelTree( ForestMap& monitorTree )
{
	try
	{
// 获取SVSE数目
...
// 遍历整个svdb，查找对应hwID设备中是否已经存在相同监测器
				...
				if( pMap != NULL )
				{
					...
					for( PAIRLIST::iterator itMType=listMonitor.begin(); itMType!=listMonitor.end(); itMType++ )
					{
						...
						if( obj != INVALID_VALUE ) 
						{
							...
							if( node != INVALID_VALUE )
							{
									FindNodeValue( node, "line", strNewLine );								
							}
							else
							{
								continue;
						...
						if( (itMap=pMap->find(itMType->value)) != pMap->end() )
						{
							string strLine = itMap->second;
							strLine += strNewLine;
							itMap->second = strLine;
						}
						else
						{
							pMap->insert( pair<string,string>(itMType->value, strNewLine) );
						}
					}
				}
			}
	...
	catch( ... )
	{
		...
		char strMsg[128] = {0};
		sprintf(strMsg , "Exception occured in CreateAGMonitorLabelTree, Error number:%d! Error msg:%s!",dwError , lpBuffer);
		LocalFree(lpBuffer);
		WriteLog(strMsg);
	}	
}
...
void CreateAddAGMonitorTree( ForestMap& fmap,  string strMonitorType )
{
	ForestMap addMonitorTree;
	ForestMap::iterator it;
	ForestMap::iterator itMonitorTree;
	...
	for( it=fmap.begin(); it!=fmap.end(); it++ )
	{
		...
		if( obj != INVALID_VALUE )
		{
			...
			{
				FindNodeValue( node, "hwID", strHWID );
				FindNodeValue( node, "lineNum", strLineNum );
			}
			else
			{
				continue;
		...
		if(strLineNum.empty())
			strLineNum = "1";
		int nLineNum = atoi( strLineNum.c_str() );
		...
		it->second.insert( pair<string,string>("hwID",strHWID) );
		it->second.insert( pair<string,string>("line",strNewLine) );

		if( (itMonitorTree=addMonitorTree.find(strHWID)) != addMonitorTree.end() )
		{
			string strLine = itMonitorTree->second[strMonitorType];

			basic_string<char>::size_type pos,off(0);
			bool bFind(false);
			while( (pos=strNewLine.find(",",off)) != basic_string<char>::npos )
			{
				if( strLine.find(strNewLine.substr(off, pos-off+1)) != basic_string<char>::npos )
				{
				}
				else
				{
					strCanLine += strNewLine.substr(off,pos-off+1); 
				}
				off = pos + 1;
			}
			it->second["line"] = strCanLine;
			if( !strCanLine.empty() )
			{
				strLine += strCanLine;
				itMonitorTree->second[strMonitorType] = strLine;
			}
			else
			{
			}
		...
		{
			map<string,string> newMap;
			newMap[strMonitorType] = strNewLine;
			addMonitorTree[strHWID] = newMap;
		}
	}
}
		...
		if( !strNewId.empty() )
		{
			...
			if( obj != INVALID_VALUE ) 
			{
				...
				if( paramNode != INVALID_VALUE )
				{
					...
					if(!strIsdn.empty())			//isdn数目不为空则加到监测器属性中
					{
						AddNodeAttrib( paramNode, "isdn", strIsdn );
					}
					else
					{
						AddNodeAttrib( paramNode , "line" , "1,");
					}
			...
			entityNode = GetEntityMainAttribNode(entityObj);
			FindNodeValue(entityNode , "sv_devicetype" , strDevType);

			SetSingleParameter( fmap, inwhat, strDevType , estr );
		}
...
bool MassAddMonitor(ForestMap& fmap,  const NodeData& inwhat, string& estr)
{
		...
		)
	{
		...
		{
			estr += "服务端添加监测器失败";
			WriteLog("MassAddMonitor failed!");
			WriteLog(estr.c_str());
			return false;
		}
		// 第二次扫描待添加监测器
		for( it=fmap.begin(); it!=fmap.end(); it++ )
		{
			...
			if( !(it->second["isdn"]).empty() )
			{
				...
				string strCanIsdn;
//				string strNewLine = it->second["line"];
		...
		(nMonitorType == 1410) ||			//AG类型的设备
		(nMonitorType == 1411) ||			//AG类型的设备
		(nMonitorType == 1412) ||			//AG类型的设备
		(nMonitorType == 1413) ||			//AG类型的设备
		(nMonitorType == 1414) ||			//AG类型的设备
		(nMonitorType == 1415) 				//AG类型的设备
		)
	{
		// 对于这6类监测器，必须先判断是否允许添加监测器
		// 建立监测器标签树
		ForestMap monitorTree;
		CreateAGMonitorLabelTree( monitorTree );

		// 第一次扫描待添加监测器
		CreateAddAGMonitorTree( fmap, strMonitorType );
		...
		for( it=fmap.begin(); it!=fmap.end(); it++ )
		{
			...
			if( (itMTree=monitorTree.find(strHWID)) != monitorTree.end() )
			{
				if( (itMMap=itMTree->second.find(strMonitorType)) != itMTree->second.end() )
				{
					string strLine = itMMap->second;

					basic_string<char>::size_type pos,off(0);
					while( (pos=strNewLine.find(",",off)) != basic_string<char>::npos )
					{
						string strTmp = strNewLine.substr(off, pos-off+1);
						if( strLine.find(strTmp) != basic_string<char>::npos )
						{
						}
						else
						{
			...
			string strNewId = AddSipMonitor( strTempId, it->first, strCanLine, "", strMonitorType, estr );
...
bool CpeTransferComplete(ForestMap &fmap , const NodeData &inwhat , string &estr)
{
	...
	{
		char strTemp[512] = {0};
		sprintf(strTemp , "%s,%s,%s,%s" , strManufacturer.c_str() , strOUI.c_str() , strProductClass.c_str() , strSerialNumber.c_str());
		if(strFaultCode.empty())
		{
			WriteIniFileString(strTemp , "UpdateState" , "升级失败" , "SipDownload.ini");
		}
		else if(atoi(strFaultCode.c_str()) == 0)
		{
			WriteIniFileString(strTemp , "UpdateState" , "升级完成" , "SipDownload.ini");
		}
		else
		{
			WriteIniFileString(strTemp , "UpdateState" , "升级失败" , "SipDownload.ini");
		}
		WriteIniFileString(strTemp , "FaultCode" , strFaultCode , "SipDownload.ini");
		WriteIniFileString(strTemp , "FaultString" , strFaultString , "SipDownload.ini");
		WriteIniFileString(strTemp , "startTime" , strStartTime , "SipDownload.ini");
		WriteIniFileString(strTemp , "completeTime" , strCompleteTime , "SipDownload.ini");
		WriteIniFileString(strTemp , "commandKey" , strCommandKey , "SipDownload.ini");
		WriteIniFileString(strTemp , "IP" , strIP , "SipDownload.ini");
		bRet = true;
	}
	catch(...)
	{
		...
		estr += lpBuffer;
		WriteLog("CpeTransferComplete Exception:");
		WriteLog(estr.c_str());
		::LocalFree(lpBuffer);
		bRet = false;
	}
...
int GetUpdatingNum()
{
	...
	{
		string strTemp = "";
		strTemp = pSV->second.find("UpdateState")->second;
...
bool SipDownLoad(ForestMap &fmap , const NodeData &inwhat , string &estr)
{
	bool	bRet = false;
	...
	string	strServerIP;
	string	strServerPort;
	string	strIp;													//即Entity里面的_MachineName
	string	strMachinePort;											//即Entity里面的_MachinePort，2009-12-16新增，以后通过ECC的设备模板将IP和端口号分开从客户端输入，在这里和其它要打SOAP包的地方按IP:Port的格式合起来
    
	string	strManufacturer	= inwhat.find("manufacture")->second;	//厂商	
	string	strProductClass	= inwhat.find("productclass")->second;	//设备类型
	string	strVer			= inwhat.find("version")->second;		//正在升级的版本，要记入到SipDownload.ini中保存起来，以便查询时返回
	string	strFileName		= inwhat.find("filename")->second;		//文件名
	string	strFileType		= inwhat.find("filetype")->second;		//文件类型
	string	strFileSize		= inwhat.find("filesize")->second;		//文件大小（以字节为单位）
//URL组成格式为：http://IP:Port/厂商/设备类型/版本/文件名

	string	strEntityIds	= inwhat.find("EnIds")->second;			//传入的ECC设备ID串，其中设备ID用","隔开，实际使用时，需要将各设备ID先解析出来
	string	strIsImmedBegin	= inwhat.find("IsImmedBegin")->second;	//是否马上开始升级
	string	strBeginTime	= inwhat.find("BeginTime")->second;		//升级开始时间，时间格式为：YYYY-MM-DD HH：MM：SS 例如：2009-10-12 14：16：32
	string	strCommandKey	= inwhat.find("CommandKey")->second;	//本次升级唯一标识（客户端传进来为一个Guid串）
	string	strURL			= inwhat.find("url")->second;			//GetIniFileString("siteCfg" , "Host" , "" , "DeviceUpgrade.ini");升级文件url

//Password、Protocal、User信息从ECC的配置文件siteCfg中获取
	string	strOUI;
	string	strSerialNumber;
	string	strUsername		= GetIniFileString("siteCfg" , "User" , "" , "DeviceUpgrade.ini");
	string	strPassword		= GetIniFileString("siteCfg" , "Password" , "" , "DeviceUpgrade.ini");
	string	strUpCount		= GetIniFileString("OtherSet" , "maxUpCount" , "" , "DeviceUpgrade.ini");			//同时升级的设备数量
	
	string	strSuccessURL	= "";		//暂时不用
	string	strFailureURL	= "";		//暂时不用

	string	CPE_UserName;				//CPE设备用户名，2009-12-15新增
	string	CPE_PassWord;				//CPE设备密码，2009-12-15新增
	list<string> entIdList;				//用以保存设备ID的链表
	const char *steps = ",";

	char entityIds[1024] = {0};
	sprintf(entityIds , "%s" , strEntityIds.c_str());
	char * strToken = strtok(entityIds , steps);

	while(strToken != NULL)
	{
		entIdList.push_back(strToken);
		strToken = strtok(NULL , steps);
	}

	OBJECT	entObj = NULL;
	MAPNODE	entNode = NULL;
	string	strDeviceID;
	string	strEntityName;
	size_t nPos = -1;

	list<string>::iterator pList = entIdList.begin();
	while(pList != entIdList.end())
	{
		entObj = Cache_GetEntity(*pList);
		entNode = GetEntityMainAttribNode(entObj);
		FindNodeValue(entNode , "hwID" , strDeviceID);
		FindNodeValue(entNode , "sv_name" , strEntityName);
		FindNodeValue(entNode , "_MachineName" , strIp);

		FindNodeValue(entNode , "UserName" , CPE_UserName);
		FindNodeValue(entNode , "PassWord" , CPE_PassWord);

		FindNodeValue(entNode , "_MachinePort" , strMachinePort);	//2009-12-16新增
		if(!strMachinePort.empty())
		{
			strIp += ":" + strMachinePort;
		}
//对密码进行解密后再传
		char strBuf[128] = {0};
		Des des;
		des.Decrypt(CPE_PassWord.c_str() , strBuf);
		CPE_PassWord.clear();
		CPE_PassWord = strBuf;
		...
		WriteIniFileString(strDeviceID , "EntityId" , *pList , "SipDownload.ini");				//ECC设备ID号，用以查找WebServiceIP和端口。
		WriteIniFileString(strDeviceID , "UpdateState" , "等待升级" , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "version" , strVer , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "EntityName" , strEntityName , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "DeviceId" , strDeviceID , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "filename" , strFileName , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "filetype" , strFileType , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "filesize" , strFileSize , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "IsImmedBegin" , strIsImmedBegin , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "BeginTime" , strBeginTime , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "CommandKey" , strCommandKey , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "Ip" , strIp , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "url" , strURL , "SipDownload.ini");

		WriteIniFileString(strDeviceID , "CPEUserName" , CPE_UserName , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "CPEPassWord" , CPE_PassWord , "SipDownload.ini");
		WriteIniFileString(strDeviceID , "DelayTime" , "15" , "SipDownload.ini");

		pList++;
	}
	int nUpdateEntNum = 0;			//升级了的设备数目，当此数目大于或者等于可同时升级的设备数时，清零，升级程序休眠10秒再执行。
	int	nSend = 0;					//当前发送升级命令次数，发送10次后，如果还是发送不出去，则返回失败
	HMODULE hDll;

//如果升级为马上开始，则直接调用升级程序，并且将SipDownload.ini中对应的Section的UpdateStatus标记为“正在升级”，否则将信息保存到SipDownload.ini中对应的section，并且标记为“等待升级”
	try
	{
		if(strIsImmedBegin.compare("true") == 0 || strIsImmedBegin.compare("True") == 0 || strIsImmedBegin.compare("TRUE") == 0)
		{	
			bRet = true;
			pList = entIdList.begin();
			while(pList != entIdList.end())
			{
//SipDownload:
				nUpdateEntNum = GetUpdatingNum();
				if(nUpdateEntNum < atoi(strUpCount.c_str()))
				{
					...
					bNodeValue = FindNodeValue(entNode , "hwID" , strTemp);
					if(!bNodeValue)
					{
						estr += *pList + " is not exist";
					...
					strOUI = strTemp.substr(0 , strTemp.find("," , 0));
					strSerialNumber = strTemp.substr(strTemp.rfind("," , strTemp.length()) + 1 ,strTemp.length());
					...
					if(hDll == NULL)
					{
						...
						estr += "don't find hwWebCommon.dll";
						WriteLog(estr.c_str());
						bRet = false;
					}
					else
					{
						...
						BOOL	bResult;
						SipDL	* pFun = (SipDL *)::GetProcAddress(hDll , "DownLoad");
						...
						{						
							estr += "\nLoad Download function in hwWebCommon.dll fail!";
							WriteLog(estr.c_str());
							bRet = false;
						}
					}				
				...
				else
				{
					...
					char strTime[128] = {0};
					sprintf(strTime , "%.4d-%.2d-%.2d %.2d:%.2d:%.2d" , ctime.GetYear() , ctime.GetMonth() , ctime.GetDay() , ctime.GetHour() , ctime.GetMinute() , ctime.GetSecond());
					strBeginTime = strTime;
					WriteIniFileString(strDeviceID , "BeginTime" , strBeginTime , "SipDownload.ini");
					if(hDll != NULL)
					{
						::FreeLibrary(hDll);
	...
	catch(...)
	{
		...
		estr += lpBuffer;
		::LocalFree(lpBuffer);
		WriteLog("SipDownLoad Exception:");
		WriteLog(estr.c_str());
		if(hDll != NULL)
		{
			::FreeLibrary(hDll);
...
bool QueryDownloadStatus(ForestMap &fmap , const NodeData &inwhat , string &estr)
{
	bool	bRet = false;
	string	strEntityId;			//ECC的设备ID
	OBJECT	entityObj = NULL;
	MAPNODE	entityNode = NULL;
	NodeData::const_iterator pNode;

	string	strEntityName;			//ECC的设备名称，通过查询ECC设备主属性获得
	string	strDeviceID;			//SIP设备的ID，即hwID，通过查询ECC设备主属性获得
	string	strStatus;				//下载状态
	string	strVersion;				//当前的版本号

	try
	{
		pNode = fmap.find("QueryDownloadStatus")->second.begin();
		while(pNode != fmap.find("QueryDownloadStatus")->second.end())
		{
			strEntityId	= pNode->second;
			entityObj	= Cache_GetEntity(strEntityId);
			entityNode	= GetEntityMainAttribNode(entityObj);

			FindNodeValue(entityNode , "sv_name" , strEntityName);
			FindNodeValue(entityNode , "hwID" , strDeviceID);

			if(strEntityName.empty() || strDeviceID.empty())		//没有获取到设备名，取下一个设备的属性
			{
				pNode++;
				continue;
			}
			...
			{
				strStatus	= GetIniFileString(strDeviceID , "UpdateState" , "" , "SipDownload.ini");
				strVersion	= GetIniFileString(strDeviceID , "version" , "" , "SipDownload.ini");
				if(strStatus.empty())
				{
					pNode++;
					continue;
				}
				PutReturnForestMap(fmap , strEntityId , "EntityId" , strEntityId);
				PutReturnForestMap(fmap , strEntityId , "EntityName" , strEntityName);
				PutReturnForestMap(fmap , strEntityId , "DeviceId" , strDeviceID);
				PutReturnForestMap(fmap , strEntityId , "UpdateState" , strStatus);
				PutReturnForestMap(fmap , strEntityId , "Now_version" , strVersion);
			}
	...
	catch(...)
	{
		...
		estr += lpBuffer;
		::LocalFree(lpBuffer);
		WriteLog("QueryDownloadStatus Exception:");
		WriteLog(estr.c_str());
		bRet = false;
	}
	return bRet;
...
bool SipReboot(ForestMap &fmap ,const NodeData &inwhat , string &estr)
{
	bool	bRet = false;
	HMODULE hDll;
	if(inwhat.empty())
	{
		estr += "input parameters is null";
		WriteLog(estr.c_str());
		return false;
	}

	string	strEntityId = inwhat.find("EntityId")->second;
	if(strEntityId.empty())
	{
		estr += "input null entityId";
		WriteLog(estr.c_str());
		return false;
	}

	string strCommandKey = inwhat.find("CommandKey")->second;
	if(strCommandKey.empty())
	{
		estr += "input null commandkey";
		WriteLog(estr.c_str());
		return false;
	}
	try
	{
		char strParas[1024] = {0};			//提交到hwWebCommon.dll中的传入参数项
		char szReturn[1024] = {0};			//提交到hwWebCommon.dll中的返回参数项
		int nSize = 1024;					//暂无实际意义。
		...
		string	strIP;
		string	strDeviceID;
		string strUserName;			//2010-1-7新增参数
		string strPassWord;			//2010-1-7新增参数

		string	strMachinePort;

		FindNodeValue(entityNode , "hwID" , strDeviceID);
		FindNodeValue(entityNode ,"_MachineName" , strIP);
		FindNodeValue(entityNode , "serverIP" , strServerIP);
		FindNodeValue(entityNode , "serverPort" , strServerPort);
		FindNodeValue(entityNode , "UserName" , strUserName);
		FindNodeValue(entityNode , "PassWord" , strPassWord);
		FindNodeValue(entityNode , "_MachinePort" , strMachinePort);	//2009-12-16新增
		if(!strMachinePort.empty())
		{
			strIP += ":" + strMachinePort;
		}
//对密码进行解密后再传
		...
		size_t nPos = strDeviceID.find("," , 0);
		strManufacturer = strDeviceID.substr(0 , nPos);

		strDeviceID.erase(0 , nPos+1);
		nPos = strDeviceID.find("," , 0);
		strOUI = strDeviceID.substr(0 , nPos);

		strDeviceID.erase(0 , nPos+1);
		nPos = strDeviceID.find("," ,0);
		strProductClass = strDeviceID.substr(0 , nPos);
		
		strDeviceID.erase(0 , nPos+1);
		strSerialNumber = strDeviceID;

		sprintf(strParas ,"ServerIP=%s$ServerPort=%s$IP=%s$Manufacturer=%s$Oui=%s$ProductClass=%s$SerialNumber=%s$CommandKey=%s$UserName=%s$PassWord=%s$",
			strServerIP.c_str(),
			strServerPort.c_str(),
			strIP.c_str(),
			strManufacturer.c_str(),
		...
		if(hDll == NULL)
		{
			estr += "Load library hwWebCommon.dll failed!";
			WriteLog(estr.c_str());
			return false;
		}

		SipDL * pFun = (SipDL *)::GetProcAddress(hDll , "ReBoot");
		if(pFun == NULL)
		{
			estr += "Get Reboot function in hwWebCommon.dll falied!";
			WriteLog(estr.c_str());
			FreeLibrary(hDll);
			return false;
		}
		bRet = (*pFun)(strParas , szReturn , nSize);
		estr += szReturn;
	}
	catch(...)
	{
