	...
	for(FLIT fit=flist.begin(); fit!=flist.end(); ++fit)
	{
		...
		if( type.compare("entity")== 0 )
		{
			...
			if( entityObj != INVALID_VALUE )
			{
				...
				FindNodeValue( GetEntityMainAttribNode(entityObj), "_DcmServerIP", strDCMIp );
				if(strDCMIp.empty())
				{
					FindNodeValue( GetEntityMainAttribNode(entityObj), "dcmManageNodeIP", strDCMIp );
				}
				FindNodeValue( GetEntityMainAttribNode(entityObj), "_DcmPort", strDCMPort );
				if(strDCMPort.empty())
				{
					FindNodeValue( GetEntityMainAttribNode(entityObj), "dcmServerPort", strDCMPort );
				}
...
bool QueryEntity(ForestMap & fmap,  const NodeData & inwhat, string & estr)
{
	...
	FindNodeValue( GetEntityMainAttribNode(obj), "_DcmServerIP", strDCMIp );
	FindNodeValue( GetEntityMainAttribNode(obj), "_DcmPort", strDCMPort );
...
bool TryToLogin(ForestMap & fmap,  const NodeData & inwhat, string & estr, bool & isTrial)
{
	...
	{
		string LoginName2= GetValueInNodeData(mit->second,	"LoginName",estr2);		
		WriteLog("*********Forestmap LoginName2***************");
		WriteLog(LoginName2.c_str());
		WriteLog("************************\n");
		if(LoginName2.compare(LoginName)==0)
		{
			section= mit->first;
	...
	WriteLog("*********Forestmap strPassword***************");
	WriteLog(strPassword.c_str());
	WriteLog("************************\n");
...
*/
	...
	if( OneDes.Decrypt(strDataNum.c_str(),strDes) &&  OneDes.Decrypt(LastData.c_str(),strDes2) )
	{
		strDataNum=strDes;
		LastData=strDes2;
		WriteLog("strDataNum is ");
		WriteLog(strDes);
		WriteLog("LastData is ");
		WriteLog(strDes2);
		if( !strDataNum.empty() && !LastData.empty() )
		{
