         ...
         // WriteLog("INVALID_VALUE4");
			...
			if(label == "WebSmsTest")
			{
				...
				if ((pos=szSmsTo.find( "DataBase")) == string.npos )
				{
					...
					for(listSmsItem = listSms.begin(); listSmsItem!=listSms.end(); listSmsItem++)
						WebSmsTest((*listSmsItem), " This is a test!这是一个测试信息！");	
				}
				else
				{
			...
			else if(label == "SmsTest")
			{
				...
				for(listSmsItem = listSms.begin(); listSmsItem!=listSms.end(); listSmsItem++)
				{
					...
					CAlertMain::TestSmsFromComm(strSmsTo, "This is a Test!这是一个测试信息！");	
				}
...
void CAlertMain::ReadAlertObjFromIni(string strSection)
{
	...
	if(nCond == 0)		//如果按整形没取出AlertCond的值，按字符串型再取一次
	{
		...
		char temp[10];
		sprintf(temp, "未取出 nCond = %d", nCond);
		WriteLog(temp);
	}
...
string CAlertMain::GetCfgFromWatchList(string strAlertindex, bool bEmail)
{
	...
	char tempbuf[128] = {0};
	sprintf(tempbuf, "m_pListWatch的大小：%d" , m_pListWatch.size());
	WriteLog(tempbuf);
	for(m_pListWatchItem = m_pListWatch.begin(); m_pListWatchItem != m_pListWatch.end(); m_pListWatchItem++)
	{
		bWatchMatch = false;
...
//获取监测器最后一条记录的dstr值 added by lifeng.wang 2010-2-9
string CAlertMain::GetMonitorRecordDstr(string strId)
{
	...
	try
	{
		...
		cout<< "####################" <<endl;
		cout<<strId.c_str()<<":\t"<<"状态："<<state<<"详细信息："<<strInfo.c_str()<<endl;
		cout<< "####################" <<endl;
