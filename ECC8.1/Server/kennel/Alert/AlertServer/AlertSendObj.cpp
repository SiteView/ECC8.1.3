...
string GB2312ToUTF8(string intext)
{
	...
	size_t pLen= intext.size();
	size_t nLength = pLen* 3;  // exactly should be:  *3/2 +1

	char *pText=new char[static_cast<int>(nLength)];
	if(pText==NULL)
		return "";
...
string UTF8ToGB2312(string intext)
{
	...
	size_t pLen= intext.size();
	size_t nLength = pLen* 3; // exactly should be:  +1
				  ...
				  string strEntityName, string strMonitorName, string strAlertReceive, int nType, int nStatu)
{
    ...
    unsigned int len = pm - data;

    len += static_cast<unsigned int>(strlen(pm))+1;
...
{	
	...
	else
	{
		...
		if( (strcmp(strEmailAdressValue.c_str(), CAlertMain::strOther.c_str()) != 0) ||
			(strcmp(strEmailAdressValue.c_str(), CAlertMain::strENOther.c_str()) != 0) )
		{
			WriteLog("============ 绑定方式 =============");
	...
	try
	{
		...
		*/
		...
		for(listEmailItem = send_list.begin(); listEmailItem!=send_list.end(); listEmailItem++)
		{
			...
			WriteLog("--------input parameter--------");
			char strBuffer[1024] = {0};
			sprintf(strBuffer , "szEmailServer=%s\n\tszEmailfrom=%s\n\tstr_mail_to=%s\n\tstr_alert_title=%s\n\tstr_alert_content=%s\n\tszUserID=%s\n\tszUserPwd=%s",
				szEmailServer.c_str(),
				szEmailfrom.c_str(),
				(*listEmailItem).str_mail_to.c_str(),
				(*listEmailItem).str_alert_title.c_str(),
				(*listEmailItem).str_alert_content.c_str(),
				szUserID.c_str(),
				szUserPwd.c_str()                
				);
			WriteLog(strBuffer);
...
void CAlertEmailSendObj::MakeAlertTitle()
{
	...
	size_t nLength = strAlertTitle.length();
	size_t nPos=strAlertTitle.find("&");
	...
	{	
		strAlertTitle = "警报来自SiteView。@Group@:@monitor@"; //old
//段毅2011-06-20解决主题中文乱码
		//strAlertTitle = "SiteView Alert.@Group@:@monitor@"; 
//段毅2010-11-15 ISTM需要监测器ID在标题里
		//strAlertTitle = "警报来自SiteView 监测器ID (";
		//strAlertTitle += strAlertMonitorId;
		//strAlertTitle += ")";
//——————————————————————
...
//	strTmp = CAlertMain::ReplaceStdString(strAlertTitle, "@Status@", strEventDes);	
	strTmp = CAlertMain::ReplaceStdString(strAlertTitle, "@Status@", CAlertMain::GetMonitorRecordState(strAlertMonitorId));
	strAlertTitle = strTmp;
...
//yi.duan 2011-02-24 需要监测器描述信息
	...
	strTmp = CAlertMain::ReplaceStdString(strAlertContent, "@MonitorAlertDes@", CAlertMain::GetMonitorRecordDstr(strAlertMonitorId));
//yi.duan 2011-02-24--------
	strAlertContent = strTmp;
	cout << "MonitorAlertDes strTmp = "<< strTmp << endl;
	//
...
//	strTmp = CAlertMain::ReplaceStdString(strAlertContent, "@Status@", strEventDes);	
	...
	//将监测器的最后一条记录的dstr值替换进来。added by lifeng.wang 2010-2-9
	strTmp = CAlertMain::ReplaceStdString(strAlertContent, "@MonitorDstr@", CAlertMain::GetMonitorRecordDstr(strAlertMonitorId));
	strAlertContent = strTmp;
	cout << "MonitorDstr strTmp = "<< strTmp << endl;
	...
	*/
	...
	for(int i=0;i!=nCondCount;i++)
	{
		...
		if(i!=0)
		{
			...
			int pos1=static_cast<int>(strtemp.find(chtemp));
			...
			int pos2 = static_cast<int>(strtemp.find(chtemp));
...
//					if(atoi(GetIniFileString((*keyitem) , "bCheck" , "0" , "emailAdress.ini").c_str()) != 0)
					{
						return false;
					}
...
//					if(atoi(GetIniFileString((*keyitem), "bCheck" , "0" , "emailAdress.ini" , "localhost" , strIdcId).c_str()) != 0)
						return false;
...
bool CAlertSmsSendObj:: SendAlert()
{
	...
	else
	{
		...
		//绑定方式
		if( (strcmp(strSmsTemplateValue.c_str(), CAlertMain::strOther.c_str()) != 0) ||
			(strcmp(strSmsTemplateValue.c_str(), CAlertMain::strENOther.c_str()) != 0) )
		{
			WriteLog("============ 绑定方式 =============");
			...
			while((index2=strTemp.find(",",index1+1))!=npos)
			{
				...
				if(GetInfoFromSmsAddress(strTemp.substr(index1,index2-index1)))
				{
					...
					sms_info.str_sms_to = szSmsTo;
//dy--------------------------------------------
					WriteLog("strSmsSendMode =");
					WriteLog(strSmsSendMode.c_str());
//dy--------------------------------------------
					if(strSmsSendMode == "Web")
					{
						MakeWebAlertContent();
...
//added by lifeng.wang at 2010-2-21
	strTmp = CAlertMain::ReplaceStdString(strAlertContent, "@Device@", CAlertMain::GetDeviceTitle(strAlertMonitorId));
	strAlertContent = strTmp;
...
{
	WriteLog("--------MakeWebAlertContent Start--------");
...
//如果找到了"\\;"
	...
	//string strGrouptId = FindParentID(strDeviceId);
	WriteLog("strTempTo:");
	WriteLog(strDeviceId.c_str());
	string strTempTo = CAlertMain::GetAllGroupTitle(strDeviceId);
	WriteLog(strTempTo.c_str());
...
//added by lifeng.wang at 2010-2-21
	...
	WriteLog("--------MakeWebAlertContent End--------");
	WriteLog(strAlertContent.c_str());
	//
			...
			for(keyitem = keylist.begin(); keyitem != keylist.end(); keyitem ++)	
			{
				...
				if(strAddressName == GetIniFileString((*keyitem), "Name", "" , "smsphoneset.ini", "localhost", strIdcId))
				{
					...
					if(CAlertMain::IsScheduleMatch(szSchedule))
					{
						...
						if(!strSmsTemplateValue.empty())
						{
							return true;
						}
						else
						{
							WriteLog("Get Template from smsphoneset.ini fail!");
							return false;
						}
					}else
					{
...
{
	//size_t pos1= 0,pos2 = 0; //yi.duan 2010-12-27 size_t 是unsigned 
	int pos1= 0,pos2 = 0;
	...
	while(1)
	{
		...
		if (pos2<0)  //这个判断一直不会进去
		{
			vOut.push_back(strIn.substr(pos1, strIn.length() -pos1));

			printf("break,break,%s\n",(strIn.substr(pos1, strIn.length() -pos1)).c_str());
			break;
		}
...
int CAlertSmsSendObj::process_msg(std::vector<std::string> &vOut,string strIn,string strFmt,int msglen)
{
	...
	if (static_cast<int>(strMsg.size()) > msglen+4)
	{
		...
		while(strMsg.size() > 0)
		{
			...
			while(count < static_cast<int>(strMsg.size()))
			{
		...
		for(int i=0; i < static_cast<int>(vTmp.size()); i++)
		{
			char szTmp[512];
...
//拼装SQL
	...
	try
	{
		...
		printf("strAlertContent=%s", strAlertContent.c_str());
//执行SQL
		for (std::vector<std::string>::iterator i = strPhones.begin(); i != strPhones.end(); i++)
		{
			printf("for strPhones\n");
			for(std::vector<std::string>::iterator j = strMsgs.begin(); j != strMsgs.end(); j++)
			{
				printf("======*****SQL:%s\n", strSql.c_str());
				otl_long_string mobile(i->c_str() , 0 , static_cast<const int>(i->size()));
				otl_long_string msg(j->c_str() , 0 , static_cast<const int>(j->size()));
				SYSTEMTIME st;
				::GetLocalTime(&st);
...
{
	bool bRet = true;
	WriteLog("---------SendSmsFromWeb Start--------");
	char strTempBuf[1024] = {0};
	...
	memset(strTempBuf , '\0' , sizeof(strTempBuf)/sizeof(char));
	sprintf(strTempBuf , "获取到的账号=%s\t密码=%s" , User.c_str() , dechar);
	WriteLog(strTempBuf);
	...
	memset(strTempBuf , '\0' , sizeof(strTempBuf)/sizeof(char));
	sprintf(strTempBuf , "获取到的短信长度=%s" , sendlength.c_str());
	WriteLog(strTempBuf);
...
{
	char szTemp[1024] = {0};
	WriteLog("============ CAlertSmsSendObj::SendSmsFromSelfDefine  =============");
	...
	strDllName = GetIniFileString( "DLL", "DllName", "",  "interfacedll.ini");
	strDllFuncParam = GetIniFileString( "DLL", "DllFunParam", "",  "interfacedll.ini");
	...
	sprintf( szTemp, "strDllName:%s,strDllFuncParam:%s\n", strDllName.c_str(), strDllFuncParam.c_str() );
	WriteLog( szTemp );
	...
	cout <<  "-------------start loadlibrary self smsdll------------------\n" << path << endl;
	...
	if(hMod != 0)
	{
		...
		if(smssend1 != 0)
		{
			sprintf( szTemp, "smsTo:%s,content:%s\n", szSmsTo.c_str(), strAlertContent.c_str() );
			WriteLog( szTemp );
...
int CAlertSmsSendObj::GetResponseCode(_bstr_t bstrResponse)
{
    ...
    if(chTemp)
    {
        ...
        if(chTemp)
        {
            int nCode = (int)strlen(chTmp) - (int)strlen(chTemp);
            memset(chResponse, 0, strlen(chResponse));
            strncpy(chResponse, chTmp, nCode);
            chResponse[nCode] = '\0';
            return atoi(chResponse);
...
{
	WriteLog("-------CAlertScriptSendObj::SendAlert() Start-------");
	//modyfy by jiewen.zhang，增加判断，当监视器不存在时，不发送告警
	string stmp=CAlertMain::GetMonitorTitle(strAlertMonitorId);
	WriteLog(strAlertMonitorId.c_str());
	WriteLog(stmp.c_str());
	if(stmp.empty())
		return false;
...
void CAlertScriptSendObj::GetUserInfoFromServer()
{
	if(strScriptServerId == "127.0.0.1")
	{
		strUserName = "";
		strUserPwd = "";
		strServerTextValue = "127.0.0.1";
		return;
	}

WriteLog("strScriptServerId ");
...
{
	FILE *fp = NULL;
	if ((fp=fopen("dyScript.log", "a"))==NULL)
	{
...
CString CAlertScriptSendObj::DoWinScript()
{
	...
	OutputDebugString("服务器信息1：");
	OutputDebugString(strServerTextValue.c_str());
	OutputDebugString(strUserName.c_str());
	OutputDebugString(strUserPwd.c_str());

	cout << "服务器信息2：" << endl 
		 << strServerTextValue.c_str() << endl
		 << strUserName.c_str() << endl
		 << strUserPwd.c_str() << endl;

	WriteLog( "服务器信息3:" );
	WriteLog( strServerTextValue.c_str() );
	WriteLog( strUserName.c_str() );
	...
	//if(SUCCEEDED(hr))
	{
		if(strScriptFile == "SendMessage.vbs")
		{
			...
			if(nParamCount == 2)
			{
				...
				cout << "参数：" << endl 
					<< strToServer.GetBuffer() << endl
					<< strLast.GetBuffer() << endl;
				...
				cout << "结果：" << strReturn.GetBuffer() << endl;
			}
		}
		...
		else if (strScriptFile == "PlayRemoteSound.vbs")
		{
			...
			WriteLog("strScriptParam = ");
			WriteLog( strScriptParam.GetBuffer() );
...
//8.1.2内蒙 脚本告警--------------------------
			...
			////	for(keyitem = keylist.begin(); keyitem != keylist.end(); keyitem ++)
			////	{
			////		string strIndex = GetIniFileString(*keyitem, "nIndex", "", "alert.ini");
			////		strScriptParam = GetIniFileString(strIndex, "ScriptParam", "", "alert.ini");
			////	}
			////}	
...
//8.1.2内蒙 脚本告警--------------------------
		//	strToServer = "D:\\Program Files\\TTPlayer\\TTPlyer.exe" ;
		//	strLast = "D:\\6.mp3";
//8.1.2内蒙 脚本告警-------------------------- 
			WriteLog( "PlayRemoteSound.vbs" );
			WriteLog( strToServer.GetBuffer() );
			WriteLog("strLast = ");
			WriteLog( strLast.GetBuffer() );
...
std::map<std::string,DWORD> CAlertItsmSendObj::m_mapFilterCache;
...
/*	if(GetCgiVersion().compare("IDC") == 0)
	...
	*/
	...
	if(port == 0)
	{
		port = atoi(GetIniFileString("itsm_server", "itsmPort", "0" , "itsmConfig.ini").c_str());
	}
//	int port = atoi(GetIniFileString("itsm_server", "itsmPort", "0" , "itsmConfig.ini").c_str());
...
{	
	...
	MonitorCacheType::iterator fd =m_mapFilterCache.find(strAlertMonitorId);

	if (fd != m_mapFilterCache.end())
	{
		if (GetTickCount() - fd->second < 30 *60*1000)
			return false;
	}

	m_mapFilterCache[strAlertMonitorId] = GetTickCount();
...
//	strTmp = CAlertMain::ReplaceStdString(strAlertContent, "@Status@", strEventDes);	
	...
	for(int i=0;i!=nCondCount;i++)
	{
		...
		if(i!=0)
		{
			...
			size_t pos1=strtemp.find(chtemp);
			...
			size_t pos2=strtemp.find(chtemp);



			size_t count=pos2-pos1-2;
			strErrorValue+=strtemp.substr(pos1+2,count);
...
bool CAlertItsmSendObj::requestJSONAction(CInternetSession *sess,string& url,string& req,string& reply)
{
	...
	try
	{
		...
		strContent = "jsondata=" + base64.Encode(req.c_str() , static_cast<int>(req.size()));
		
		fileGet->SendRequest("",0,(LPVOID)(LPCTSTR)strContent.c_str(),static_cast<DWORD>(strContent.size()));
		fileGet -> QueryInfoStatusCode(dwStatusCode);
		if(dwStatusCode == HTTP_STATUS_OK)   
		{
			iTryCount = 0;
...
public:
	...
	char result(const char *expression)
	{
		...
		while(i < static_cast<int>(strdesc.size()))
		{
			char c= strdesc[i];
			++i;
...
bool CAlertItsmSendObj::MatchCondition(string strMonitorId, string strCondition)
{
	...
	for(ConditionMap::iterator ci = conditions.begin(); ci != conditions.end(); ci++)
	{
		...
		for(int j = 0; j < static_cast<int>(strdesc.size()); j++)
		{
			if (strdesc[j] == '{')
			{
				strMonID = "{";
			...
			if(strdesc[j] == '}')
			{
				...
				}else
				{
					...
					}else
					{
						...
						//看该检测器的最后状态是不是有效的（监测频率加了30分钟的缓冲时间）
						if(static_cast<int>(dwNow - dwTime) > (nFreq+30) * 60 * 1000) //
						{	
							strTmpDesc.push_back('0');
						}else
						{
...
{	
	strAlertContent = "警报来自SiteView。监测器: @AllGroup@:@Group@:@monitor@ \
状态: @Status@  时间: @Time@";
}
...
{
	WriteLog(">>>>>>>>SebSendObj::MakeAlertContent Start<<<<<<<<<");
	WriteLog("初始strAlertContent内容：");
	WriteLog(strAlertContent.c_str());
	string strTmp;

	strTmp = GetIniFileString("web", "huaWei", "", "TxtTemplate.Ini");
...
//	strTmp = CAlertMain::ReplaceStdString( strAlertContent, "@Status@", strEventDes );
	...
	strTmp = CAlertMain::ReplaceStdString( strAlertContent, "@Time@", strTime );	
	strAlertContent = strTmp;
	WriteLog("替换后strAlertContent内容：");
	WriteLog(strAlertContent.c_str());
	WriteLog(">>>>>>>>SebSendObj::MakeAlertContent End<<<<<<<<<");
...
bool CAlertWebSendObj::SendAlert()
{
	...
	try
	{
		HINSTANCE hDll = ::LoadLibrary( "huaWeiWeb.dll" );

		if( hDll != NULL )
		{
			WEBDLL* pFun = ( WEBDLL* )::GetProcAddress( hDll, "" );
			if( pFun != NULL )
			{
				try
				{
