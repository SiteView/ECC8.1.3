...
//    char TP_UD[161];    // 原始用户信息(编码前或解码后的TP-UD)
	...
	sprintf( szTemp, "报警内容：%s\n\0", strTemp.GetBuffer(strTemp.GetLength()) );
	WriteLog( szTemp );

	for(int iIndex=0 ;iIndex < iPage;iIndex++)
	{
		sprintf( szTemp, "短信内容：%s\n\0", msglist->chMsg );
		WriteLog( szTemp );
        ...
        nLength = ReadPort(ans, 128);
        ans[nLength] = '\0';
		sprintf( szTemp, "AT+CSMS=0的应答：%s\n\0", ans );
		WriteLog( szTemp );

		sprintf(cmd, "AT+CMEE=1\r");      
        WritePort(cmd, strlen(cmd));  
 	    Sleep(ShortSleepTime);
        nLength = ReadPort(ans, 128);
        ans[nLength] = '\0';
		sprintf( szTemp, "AT+CMEE=1的应答：%s\n\0", ans );
		WriteLog( szTemp );
...
int CSerialPort::page(char *SrcMsg, MsgList *DestMsg, int nSMSMaxLength )
{
	...
	char szTemp[1024] = {0};
	sprintf( szTemp, "设置的长度：%d, 实际的长度：%d\0", nSMSMaxLength, nMax );
	WriteLog( szTemp );
