	...
	while(!bStop)
	{
		...
		if(!m_AlertEventList.empty())
		{
			...
			if(lastEventobj != NULL)
			{
				...
				{
/*------------------------------为只报警一次的情况设计 Start------------------------------------------*/
					if((*m_AlertObjListItem)->nCond == 2)				//nCond为2，则只发一次报警
					{
						strTempSection = (*m_AlertObjListItem)->strIndex;
						WriteLog(strTempSection.c_str());
						string strTempCountList;
						string strAlertTargerList;
						list<string> monitorList;
						list<string> counterList;
						strTempCountList = GetIniFileString(strTempSection, "TempCount", "", "alert.ini");
						WriteLog(strTempCountList.c_str());
						strAlertTargerList = GetIniFileString(strTempSection, "AlertTarget", "", "alert.ini");
						WriteLog(strAlertTargerList.c_str());
						m_AlertCount.clear();
						if(!strTempCountList.empty())
						{
							char strTempMonitor[10240] = {0};
							char strTempCounter[1024] = {0};
							sprintf(strTempMonitor , "%s" , strAlertTargerList.c_str());
							sprintf(strTempCounter , "%s" , strTempCountList.c_str());

							char seps[]   = ",";
							char * token;
							token = strtok(strTempMonitor, seps);
							...
							token = strtok(strTempCounter , seps);
							while(token != NULL)
							{
								counterList.push_back(token);
								token = strtok(NULL , seps);
							}

							list<string>::iterator pMonList = monitorList.begin();
							list<string>::iterator pCounterList = counterList.begin();

							for(;pMonList != monitorList.end() ;pMonList++ , pCounterList++)
							{
								m_AlertCount[*pMonList] = *pCounterList;
							}
						}

						string strNewCount;
						int nTempCount = 0;
						char strTempCount[128] = {0};

						p_AlertCount = m_AlertCount.begin();						
						while(p_AlertCount != m_AlertCount.end())
						{
							memset(strTempCount , '\0' , sizeof(strTempCount)/sizeof(char));
							if(p_AlertCount->first.compare(lastEventobj->strMonitorId.c_str()) == 0)
							{
								nTempCount = atoi(p_AlertCount->second.c_str());
								nTempCount++;
								lastEventobj->nEventCount = nTempCount;								
								sprintf(strTempCount , "%d" , nTempCount);
								p_AlertCount->second = strTempCount;
							}
							sprintf(strTempCount , "%s," , p_AlertCount->second.c_str());					
							strNewCount += strTempCount;
							p_AlertCount++;
						}
						WriteIniFileString(strTempSection , "TempCount" , strNewCount , "alert.ini");
					}
/*------------------------------为只报警一次的情况设计 End------------------------------------------*/
	...
	while(1)
	{
		...
		if (mrd == INVALID_VALUE)
		{
			...
			if (NoMsgCount > 160) //线程60次没有没有取到消息
			{
				if (poolMutex->test())
				{
					if (workthreads.size() > m_nBaseThreadCount)
					{
						printf("@@@@@@@@@@@线程太多，退出一个 @@@@@\n"); 
...
/*----------------为只发送一次报警设计 Start----------------
	if(nCond == 2)
	{
		list<string> monitorList;
		list<string> counterList;
		strTempCountList = GetIniFileString(strSection, "TempCount", "", "alert.ini");
		if(!strTempCountList.empty())
		{
			char strTempMonitor[10240] = {0};
			char strTempCounter[1024] = {0};
...
/*----------------为只发送一次报警设计 Start----------------
	if(nCond == 2)
	{
		strTempCountList = GetIniFileString(strIndex, "TempCount", "", "alert.ini", "localhost", strIdcId);
		if(!strTempCountList.empty())
		{
			char * strTempMonitor = NULL;
			char * strTempCounter = NULL;
			strTempMonitor = new char[strlen(strAlertTargerList.c_str())];
			sprintf(strTempMonitor , "%s" , strAlertTargerList);
			char strStep[] = ",";
			char * strMonitorToken;
			char * strCounterToken;
			strMonitorToken = strtok(strTempMonitor, strStep);
			strCounterToken = strtok(strTempCounter , strStep);
			while(strMonitorToken != NULL)
			{
				m_AlertCount[strMonitorToken] = strCounterToken;

				strMonitorToken = strtok(NULL , strStep);
				strCounterToken = strtok(NULL , strStep);
			}
		}
	}
/*----------------为只发送一次报警设计 End----------------*/
