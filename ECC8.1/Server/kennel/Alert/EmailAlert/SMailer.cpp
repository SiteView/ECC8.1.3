      ...
      _user_pwd(user_pwd), _mail(0)
{
    ...
    hello();*/
	WriteLog("MailSender::MailSender");
...
void MailSender::GetMailBaseInfo(const std::string server_name, 
               ...
               const std::string user_pwd )
{
	WriteLog("--------GetMailBaseInfo Start--------");
	char strTemp[1024] = {0};
	try
	{
		_server_name = server_name;
		...
		sprintf(strTemp , "_server_name=%s\t_user_name=%s\t_user_pwd=%s", _server_name.c_str() , _user_name.c_str() , _user_pwd.c_str());
		WriteLog(strTemp);
		conn();

		if ( !user_name.empty() )// 处理需要进行安全验证的SMTP服务器
		{
			login();
		}
		else// 支持匿名发送的SMTP服务器
		{
			...
			{
				sprintf(strTemp , "local_host=%s" , local_host);
				WriteLog(strTemp);
...
void MailSender::conn()
{
	...
	sprintf(strTemp , "_server_name=%s", _server_name.c_str());
	WriteLog(strTemp);
...
{
	WriteLog("=========== MailSender::sendHeader ==========");
	...
	catch(...)
	{
		...
		char ErrBuff[512] = {0};
		sprintf(ErrBuff , "errorId=%d ,error info=%s" , nErrId , strErrInfo);
		::LocalFree(strErrInfo);
		WriteLog(ErrBuff);	
	}
}
...
{
	WriteLog("=========== MailSender::sendContent ==========");
...
bool MailSender::rcvResponse(const std::string expected_response)
{
	...
	char szTemp[1024] = {0};
	sprintf( szTemp, "[INFO]RECEIVE:%s", response_buffer );
	WriteLog( szTemp );
...
//        throw MailException(ErrorMessage::getInstance().response(expected_response));
 ...
 //             << response << std::endl;
	sprintf( szTemp, "reponse:%s", response.c_str() );
	WriteLog( szTemp );
                             ...
                             const std::string content)
{
	...
	char szTemp[1024] = {0};
	sprintf( szTemp, "[INFO]SEND:%s", content.c_str() );
	WriteLog( szTemp );
