	...
	try{
		...
		if(pos == NULL)
		{
			...
			while(*pos != '\0')
			{
				...
				int nMid = nSize / 2;
				std::string::size_type spos,off(nMid);
//dy 梅州 UNIX进程下拉菜单中有=号，监测器就换成了‘&’。
				if ( (spos=input.find("&",off))!=std::string::npos ||
					(spos=input.rfind("&",off))!=std::string::npos  )
				{
					off= spos+1;
...
//dy 梅州 2010-07-12
				else if( (spos=input.find("=",off))!=std::string::npos || 
					(spos=input.rfind("=",off))!=std::string::npos )
				{
					off= spos+1;
					key= input.substr(0,spos);
...
bool GetDynamicData(ForestMap & fmap,  const NodeData & inwhat, string & estr)
{
	...
	if(ret)
	{
		...
		//char strTemp[512] = {0};    //dy 梅州那边有的进程大于1024 2010-07-12
		char strTemp[4096] = {0}; 
		while(pMap != fmap.end())
		{
			sprintf(strTemp , "[%s]" , pMap->first.c_str());
			WriteLog(strTemp);
			NodeData::const_iterator pNode = pMap->second.begin();
	...
	{	
		...
		if( sv_type.compare("password")==0 ) 
		{
			...
			if( FindNodeValue( ma2,"sv_name",sv_name) )
			{
				...
				myPrintLog(sv_name.c_str());
			}
		}
//dy 调试
		//myPrintLog("-------------pairList------------------");
		//PAIRLIST pairList;
		//EnumNodeAttrib(ma2, pairList);
		//PAIRLIST::iterator pList = pairList.begin();
		//while(pList != pairList.end())
		//{
		//	myPrintLog(pList->name.c_str());
		//	myPrintLog(pList->value.c_str());
		//	pList++;
		//}
		//myPrintLog("-------------pairList------------------");
	}
...
{
	BOOL bReturn = true;  //+dy 2010-06-10
	...
	else if(type.compare("monitor")==0)
	{
		...
		OBJECT obj1= Cache_GetMonitorTemplet(atoi(sv_monitortype.c_str()));//+ dy 2010-06-10
...
//dy+ ------------------------否则取不到高级参数--------------------
		//if (section=="advance_parameter")
		//{
		//	if( ! FindMTAdvanceParameterFirst( obj2, param ) )
		//	{
		//		estr+=  "  FindMTAdvanceParameterFirst failed.   ";
		//		return false;
		//	}
		//}
		//else 
...
//dy+ --------------------------------------------2010-05-04
		...
		{
			bReturn = false;   //+ dy 2010-06-10
			//return false;    // 原来的，直接返回，advanced_parameter 就取不到,而且没必要有那么多出口.
		}
		else
		{
			bReturn = true;
			//myPrintLog("Parameter need ");
		}
		...
		{
			bReturn = true;
			//myPrintLog("AdvanceParameter need ");
		}
...
void myPrintLog(const char *LogMes)
{
	...
	FILE *fp = NULL;
	if ((fp=fopen("dyDecrypt.log", "a"))==NULL)
	{
...
bool TestEntity2(NodeData & ndata, const NodeData & inwhat, string & estr)
{
	...
	if(szDllName.empty() || szFuncName.empty())
	{
		estr += "Absent dll name or function name in the Entity templete ";
		return false;
	}
