...
//		sprintf(tempchar,"(Return_%d)%s",index,id.c_str());
		sprintf(tempchar , "(Return_%d)%s" , tempIndex , id.c_str());/*modified by lifeng.wang at 2009.9.3*/
...
bool StatRecords(string id, int byCount, TTime begintime, TTime endtime, MonitorStat & ms, ForestMap & fmap, string & estr, string dstrStatusNoNeed, bool willcompress=true, bool dstrNeed=false)
{
	...
	std::list<string> flist;
	std::list<string>::iterator it;
	if(!::GetReocrdSetField(rds,flist))		//获取 rds 对象 中的字段名
	{
		if(rds != NULL)
		{
			CloseRecordSet(rds);
	...
	if(!::FindRecordFirst(rds,litem))
	{
		...
		if(litem.list != INVALID_VALUE || litem.iterator != NULL)
		{
			ReleaseItemList(litem);
		}
		return false;
	...
	ms.SetFieldCount(flist.size());		//记录的字段数
	int findex(0);
	...
	while((rdobj=::FindNextRecord(litem))!=INVALID_VALUE)
	{
		...
		TTime time1;  
		string creattime;
		if ( GetRecordCreateTime(rdobj, time1 ) )	//获取一个记录的创建时间
		{
			try
			{ 
	...
	WriteLog("--------StatRecords End--------");
	return true;
}

//将idstr里面包含的监测器ID，检验真实有效后，放在set里面，如果是设备ID，则将其下面的所有监测器ID检验后放在set里面。
bool PrepareMonitorId(string idstr, set<string> & id )
{
	...
	while(  std::getline(input_temp,tempkey)  )
	{
		...
		OBJECT obj= Cache_GetMonitor(tempkey);	//验验是否为监测器ID，是的话直接保存在id里面
		if(obj!=INVALID_VALUE)
		{
			id.insert(tempkey);
			continue;
		...
		PAIRLIST retlist;
		GetAllMonitorsInfo(retlist,"sv_name", tempkey);	//如果tempkey是组ID或者设备ID的话，则获取其下面的所有监测器ID
		for(PAIRLIST::iterator it= retlist.begin(); it!= retlist.end(); ++it)
		{
			string mid= it->name;
			if(!mid.empty())
...
{
	WriteLog("---------QueryReportData Begin--------");
	//test1();
	//return true;
	string estr2;
	TTime btime,etime;
	...
	try
	{
		...
		{
			string tempkey= *sit;		//tempkey则指监测器ID
