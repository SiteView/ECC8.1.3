...
bool SetFmapOfRecords(MonitorStat & ms, ForestMap & fmap, string & estr, bool dstrNeed=false)
{
	NodeData ndata;
...
bool StatRecords(string id, int byCount, TTime begintime, TTime endtime, MonitorStat & ms, ForestMap & fmap, string & estr, bool willcompress=true, bool dstrNeed=false)
{
	RECORDSET rds;
	if(byCount<1)
...
bool QueryReportData(ForestMap & fmap,  const NodeData & inwhat, string & estr)
{
	...
	try{
		...
		for(set<string>::iterator sit=setid.begin(); sit!=setid.end(); ++sit)
		{
			...
			if( StatRecords(tempkey, byCount, btime, etime, ms, fmap, estr, willcompress, dstrNeed) )
				SetFmapOfRecords(ms,fmap,estr, dstrNeed);
