...
bool Queue::GetQueueAllMessageLabels(std::list<string> & retlist)
{
	...
	Page *ppage=m_pagepool->Get(m_top.m_page,false);
	if(ppage==NULL)
		return false;
	m_pagepool->Put(ppage,true,false);
	...
	int pos= m_top.m_pos; //m_top.m_pos  //m_last.m_pos

	char *pc=ppage->m_data + pos;
	if(((LPQUEUEHEAD)pc)->m_cblen!=sizeof(QUEUEHEAD))
		return false;

	S_UINT len=((LPQUEUEHEAD)pc)->m_datalen;
	len+=sizeof(QUEUEHEAD);
	...
	QueueRecord prd;
	//if(len<flen)
	//	GetLabelData(map, prd, pc, len);

	while(true)
	{
		int rlen=0,prep=0,blen;

		blen= ((LPQUEUEHEAD)pc)->m_datalen;
		blen+=sizeof(QUEUEHEAD);
		GetLabelData(retlist, prd, pc, blen);

		//svutil::buffer buf;
		//if(!buf.checksize(blen))
		//	return false;
		//char * ptbuf= buf;

		//rlen=GetSingleRecord(ppage,pos,ptbuf,blen);
		//if(rlen<0)
		//	continue;
		//ptbuf+=rlen;
		//blen-=rlen;
		//GetLabelData(map, prd, ptbuf, blen);

		prep=((LPQUEUEHEAD)pc)->m_nextrecord.m_pos;//m_prerecord.m_pos;  //m_nextrecord.m_pos
		if(prep>=0)
		{
			cout<<"prep>=0 "<<endl;
		...
		{
			Page *ptem=GetPreRecordPage(ppage);
			m_pagepool->Put(ppage,true,false);
			if(ptem==NULL)
				break;
			ppage=ptem;
...
#include <sstream>
bool Queue::GetLabelData(std::list<string> & retlist,QueueRecord & prd,const char * pt,S_UINT len)
{
	...
	//string key= prd.GetCreateTime().Format() + prd.GetLevel();
