...
import com.siteview.ecc.monitorbrower.CVBean;
...
//			}
		...
		for (int i = 0; i < size; ++i) {
			...
			try {
				...
				for (String kv : kvs){
					...
					if (keyvalue.length != 2)throw new Exception("key not found!");
					if (kv.startsWith(key)){
						if (keyvalue.length == 2){
							dstrmap.put(key, keyvalue[1]);
							getlc(keyvalue[1], li);
							exist = true;
							break;
						}
				...
				if (exist == false) throw new Exception("value not found!");
				/*
	...
	private Listheader getlh(String title, Listhead l) {
		...
		if ("时间".equals(title) || "名称".equals(title) || "描述".equals(title)
				|| title.length() > 10) {
			le2.setWidth("140px");
	...
	private void doRuntimeReport() {
		...
		tp.addDescription("正常运行时间(%)", simpleReport
