...
//			return;
			// ***************************************************
			  ...
			  ListBean result =  DictionaryFactory.getIAlertLogDaoOfSQL().queryAlertLog(new
			  AccessControl(), queryCondition);
			  ChartUtil.clearListbox(this.getAlertLogListbox()); 
//			  if (result.getList().size() == 0) throw new Exception("没有查询到数据");
			  try{
				  result.getList().size();
			  }catch(Exception e){
			  ...
			  for(Map<String, String> map : result.getList()) {
				  ...
				  Listitem listitem =BaseTools.setRow(this.getAlertLogListbox(), alertLogItem,DATE_TO_STRING.format(alertLogItem .getAlertTime()),
				  alertLogItem.getAlertName(), alertLogItem.getEntityName(),
				  alertLogItem.getMonitorName(),
				  alertLogItem.getAlertType().getComponent(),
				  alertLogItem.getAlertReceiver(), alertLogItem.getAlertStatus().getComponent()); 
