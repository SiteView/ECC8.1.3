...
public class StatsReport implements IEccProgressmeter
{
	...
	private boolean createReport(String type) throws IOException
	{
		...
		try
		{
			...
			downLoadUrl=getDownLoadUrl(getReportFileID(),type);
			String file=getCreateFile(getReportFileID(),type);
			try{
				if ("html".equals(type))
				{
	...
	public static String getCreateFile(String reportFileID,String type)
	{
	...
	public static String getHtmlFolderName(String reportFileID){
		String fileName=Toolkit.getToolkit().formatDate(new Date(Long.parseLong(reportFileID)),"yyyyMMdd_")+reportFileID;
		return Constand.statreportsavepath + fileName+File.separator;
	}
	public static String getDownLoadUrl(String reportFileID,String type)
	{
		String fileName=Toolkit.getToolkit().formatDate(new Date(Long.parseLong(reportFileID)),"yyyyMMdd_")+reportFileID;
		...
		if ("html".equals(type))
		{
			return "/main/report/showReport.jsp?reportGenID="+reportFileID+"&fileType="+type+"&currentPage=1";
