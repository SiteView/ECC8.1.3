...
public class ReportGenItem extends SvdbItem{
	...
	public ReportGenItem(ReportItem reportItem,String genID,Map<String,String> propMap)
	{
		...
		if ("html".equals(getFileType()))
			super.getPropMap().put("valid",String.valueOf(new File(StatsReport.getHtmlFolderName(genID)).exists()));
		else
			super.getPropMap().put("valid",String.valueOf(new File(StatsReport.getCreateFile(genID, getFileType())).exists()));
