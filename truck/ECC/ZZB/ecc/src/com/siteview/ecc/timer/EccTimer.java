...
public class EccTimer extends Timer {
	private Map<String, TimerListener> timerListener = new ConcurrentHashMap<String, TimerListener>();
...
//	}
	...
	private IQueueEvent LastChangeDetailEventForControl = null;
	...
	public TimerListener getTimerListener(String key){
		return timerListener.get(key);
	}
	
	public void fireEvent(IQueueEvent event)  {
		if (event instanceof ChangeDetailEvent){
			LastChangeDetailEventForControl = event;
		}
		updateData();
		for (String key : getEccTimerOrder()) {
			try {
				if ("eccControlPanel".equals(key)) continue;
				TimerListener l = getTimerListener(key);
				if (l != null)
					synchronized(l){
						l.notifyChange(event);/* 通知改变 */
					}
