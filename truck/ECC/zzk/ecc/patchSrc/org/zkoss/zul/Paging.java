...
public class Paging extends XulElement implements org.zkoss.zul.api.Paging, Paginal {
	...
	private boolean iconGrid = false;
	
	private int rowSize = 0, allsize = 0;
	
	public boolean isIconGrid() {
		return iconGrid;
	}

	public void setIconGrid(boolean iconGrid,int rowSize,int allsize) {
	...
	protected String getInfoText() {
		...
		if (isIconGrid()){
			lastItem = (_actpg+1) * _pgsz  * rowSize;
			return "[ " + (_actpg * _pgsz * rowSize + 1) + (!"os".equals(getMold()) ? " - " + (lastItem > rowSize ? rowSize : lastItem) : "")+ " / " + allsize + " ]";
		}
