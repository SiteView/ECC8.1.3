...
//		btnRightActionVisible.setClicked(false);
		...
		westTree.setSize(btnLeftTreeVisible.isClicked() ? "196px" : "0px");
		actionEast.setSize("0px");
...
//		});
		...
		timer.setDelay(1000 * Integer.parseInt("30"));
				...
				if(session.getAttribute(ConstantValues.RefreshMonitorId)!= null ){
					...
					if(eccTreeItem.getValue().getSvId().equals(id)){
						...
						if(this.monitor_listbox != null){
							...
							this.monitor_listbox.setSelectedIndex(size-1);
						}
	...
	public void refresh() {
		...
		if(doMap != null){
			...
			if("addDevice".equals(dowhat)){
				...
				EccTreeItem item = selTreeItem.getChildRen().get(size - 1);
