...
//		Executions.sendRedirect(smallUrlValue.getValue());
		...
		Pattern regex = Pattern.compile("^http://([1-9]|[1-9]\\d|1\\d{2}|2[0-1]\\d|22[0-3])(\\.(\\d|[1-9]\\d|1\\d{2}|2[0-4]\\d|25[0-5])){3}(:)([0-9]{1,6})(/ecc)$");
...
//		Object base64code = Executions.getCurrent().getDesktop().getSession().getAttribute("Session_Username");
//		String url = smallUrlValue.getValue();
//		if(base64code!= null){
//			url = smallUrlValue.getValue()+"/index.jsp?username="+base64code;
//		}
//		Executions.getCurrent().sendRedirect(url, "_blank");
		
		//上面 这种方式是成功的  //换成用 post 方式来获取 数据
		
		String url = smallUrlValue.getValue();
		Object base64code = Executions.getCurrent().getDesktop().getSession().getAttribute("Session_Username");
		if(base64code!= null){
			session.setAttribute("username", base64code.toString());
		}
		Executions.getCurrent().sendRedirect(url, "_blank");
