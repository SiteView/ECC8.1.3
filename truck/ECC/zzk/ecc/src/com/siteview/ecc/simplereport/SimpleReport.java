...
public class SimpleReport extends GenericForwardComposer
{
	...
	public SimpleReport() throws Exception
	{
		...
		try
		{
			sessionId = Manager.forceLoginAsAdmin();
			View w = SVDBViewFactory.getView(sessionId);
			if (w == null)
			{
				throw new Exception( "未登录或无效的会话！");
			}
			INode n = w.getNode(monitorId);
			if (n == null)
			{
				throw new Exception( "节点不存在或无权访问！");
			}
			if (!n.getType().equals(INode.MONITOR))
			{
				throw new Exception( "节点类型非法！");
			}
		...
		} catch (Exception e)
		{
			...
			if (sessionId != null)Manager.invalidateView(sessionId);
			return;
		}
	...
	public static BufferedImage buildCategoryBufferImage(String title, String subtite, String valuelabel, DefaultCategoryDataset data, double step, double max, Date maxdate, double min, boolean xlabel,int pngwidth,int pngheight)
	{
