...
public class ActionPopup extends TooltipPopupBase {
	...
	private Row addRow(EccTreeItem eccTreeitem, String action) {
		action = action.trim();
		boolean flag = false;
		UserRight userRight = getUserRight();
		if(userRight.isAdmin()){
			flag = true;
		}else{
			IniFile iniFile = userRight.getUserIni();
			String rightValues = iniFile.getValue(userRight.getUserid(),eccTreeitem.getId());
			if(rightValues != null && !"".equals(rightValues)){
				if(INode.SE.equals(eccTreeitem.getType())){
					if(rightValues.contains("telebackup")){//小猫
						rightValues = rightValues.substring(0, 62);
					}else{
						rightValues = rightValues.substring(0, 49);
					}
					if(rightValues.contains(action+"=1")){
						flag = true;
					}else{
						flag = true;
					}
					...
					if(rightValues.contains("telebackup")){//小猫
						rightValues = rightValues.substring(63,125);
					}else{
					...
					String entity_str = "addmonitor,editdevice,testdevice,copydevice,pastemonitor,deldevice,devicerefresh";
					if(entity_str.contains(action)){
						String tempRight = action+"=1";
						if(rightValues.contains(tempRight)){
							flag = true;
						}
					}else{
						flag = true;
					...
					String monitor_str = "editmonitor,copymonitor,delmonitor,monitorrefresh";
					if(monitor_str.contains(action)){
						String tempRight = action+"=1";
						if(rightValues.contains(tempRight)){
							flag = true;
						}
		...
		String actionName = EccActionManager.getInstance().getActionName(action);
