...
//				}
		...
		} else if (action.equals("devicerefresh") || action.equals("monitorrefresh"))
		{
			if (node.getType().equals(INode.ENTITY))
			{
				...
				if (mcount == 0)
				{
					Messagebox.show("没有可以刷新的监测器！", "提示", Messagebox.OK, Messagebox.INFORMATION);
					return;
				}
		...
		} else if (action.equals("delgroup"))
		{
			try
			{
				if("二层交换机,三层交换机,路由器,防火墙,服务器,PC设备,其他设备".contains(node.getName())){
					Messagebox.show("该设备组与NNM关联，不允许删除", "提示", Messagebox.OK, Messagebox.INFORMATION);
					return;
				}
				int r = Messagebox.show("确认删除组：" + node.getName(), "询问", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION);
		...
		} else if (action.equals("deldevice"))
		{
			try
			{
				int r = Messagebox.show("确认删除设备：" + node.getName(), "询问", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION);
				Boolean delsuccess = false;
				if (r == Messagebox.OK)
				{
					Session session = Executions.getCurrent().getDesktop().getSession();
		...
		} else if (action.equals("delmonitor"))
		{
			try
			{
				int r = Messagebox.show("确认删除监测器：" + node.getName(), "询问", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION);
				Boolean delsuccess = false;
				if (r == Messagebox.OK)
				{
					Session session = Executions.getCurrent().getDesktop().getSession();
		...
		} else if (action.equals("startup"))
		{
			try
			{
				...
				if("group".equals(node.getType())){
					r = Messagebox.show("是否启动组：" + node.getName(), "询问", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION);
				}else if("entity".equals(node.getType())){
					r = Messagebox.show("是否启动设备：" + node.getName(), "询问", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION);
				}else if("monitor".equals(node.getType())){
					r = Messagebox.show("是否启动监测器：" + node.getName(), "询问", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION);
				}
					...
					if (node.getType().equals(INode.GROUP))
					{
						...
						try
						{
							if(parentnode.getStatus().equals(INode.DISABLE)){
								Messagebox.show("父节点被禁用", "提示", Messagebox.OK, Messagebox.INFORMATION);
								return;
							}
		...
		} else if (action.equals("pastedevice"))
		{
			...
			try
			{
				...
				if (pasternode == null)
				{
					Messagebox.show("请先复制好设备！", "提示", Messagebox.OK, Messagebox.EXCLAMATION);
					return;
				}
				...
				int r = Messagebox.show("是否粘贴设备：" + pasternode.getName(), "询问", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION);
				...
				if (entityid != null && !entityid.isEmpty())
				{
					...
					if(!view.isAdmin()){
						...
						String rightValues = iniFile.getValue(userRight.getUserid(),sourceEntityId);
						if(rightValues != null){
							try{
								iniFile.setKeyValue(userRight.getUserid(), node.getSvId(), rightValues);
								iniFile.saveChange();
								System.out.println("----------增加Entity权限_粘贴："+node.getSvId()+"--"+rightValues);
							}catch(Exception e){
								e.printStackTrace();
							}
						}
				...
				Messagebox.show("粘贴设备成功！", "提示", Messagebox.OK, Messagebox.INFORMATION);
				...
				if (childids != null)
				{
					...
					if(!view.isAdmin()){
						UserRight userRight = Toolkit.getToolkit().getUserRight(Executions.getCurrent().getDesktop());
						IniFile iniFile = userRight.getUserIni();
						String rightValues = "editmonitor=1,copymonitor=1,delmonitor=1,monitorrefresh=1";
							try{
								iniFile.load();
								for(String tempId:idlist){
									iniFile.setKeyValue(userRight.getUserid(), tempId, rightValues);
								}
								iniFile.saveChange();
							}catch(Exception e){
								e.printStackTrace();
		...
		} else if (action.equals("pastemonitor"))
		{
			...
			try
			{
				...
				if (pasternode == null)
				{
					Messagebox.show("请先复制好监测器！", "提示", Messagebox.OK, Messagebox.EXCLAMATION);
					return;
				}
				...
				int r = Messagebox.show("是否粘贴监测器：" + pasternode.getName(), "询问", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION);
				if (r == Messagebox.CANCEL)
				{
					return;
				}
				...
				if (monitorid != null && !monitorid.isEmpty())
				{
					...
					if(!view.isAdmin()){
						UserRight userRight = Toolkit.getToolkit().getUserRight(Executions.getCurrent().getDesktop());
						IniFile iniFile = userRight.getUserIni();
						String rightValues = iniFile.getValue(userRight.getUserid(),sourceMonitorId);
						if(rightValues != null && !"".equals(rightValues)){
							try{
								iniFile.load();
								System.out.println(userRight.getUserid()+"#"+ node.getSvId()+"#"+rightValues);
								iniFile.setKeyValue(userRight.getUserid(), node.getSvId(), rightValues);
								iniFile.saveChange();
								System.out.println("增加 monitor 权限_粘贴："+node.getSvId()+"--"+rightValues);
							}catch(Exception e){
								e.printStackTrace();
							}
						}
				...
				Messagebox.show("粘贴监测器成功！", "提示", Messagebox.OK, Messagebox.INFORMATION);
		...
		} else if (action.equals("batchDel"))
		{
			try
			{
				...
				{
					Messagebox.show("没有选中的项目", "提示", Messagebox.OK, Messagebox.INFORMATION);
					return;
				}
				int r = Messagebox.show("确认删除选中的项目吗？", "询问", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION);
				if (r == Messagebox.OK)
				{
		...
		} else if (action.equals("batchStartup"))
		{
			try
			{
				...
				// if (starts==0)
				// {
				// Messagebox.show("所有选中的项目均已启动!", "提示", Messagebox.OK, Messagebox.INFORMATION);
				// return;
				// }
				int r = Messagebox.show("是否启动选中的项目的监测?", "询问", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION);
				if (r == Messagebox.OK)
				{
		...
		} else if (action.equals("batchDisable"))
		{
			...
			// if (starts==0)
			// {
			// Messagebox.show("所有选中的项目均已禁止!", "提示", Messagebox.OK, Messagebox.INFORMATION);
			// return;
			// }
			final Window win = (Window) Executions.createComponents(DisableMonitors_TargetUrl, null, null);
			win.setAttribute("inode", eccTreeItem.getValue());
