...
var fisheyemenu = {
	...
	imgType : ".png",
	init : function () {
		var animElements = document.getElementById("fisheye_menu").getElementsByTagName("div");
		...
		for(var i=0; i<animElements.length; i++) {
			...
			animElements[i].onclick = changeSize;
	...
	imgSmall : function (obj) {
		imgSrc = obj.childNodes.item(0).getAttribute("src");
		var smallPos = imgSrc.indexOf("_small", 0);
		if (smallPos>0) return;
		var typePos = imgSrc.indexOf(fisheyemenu.imgType, 0);
		var imgName = imgSrc.substr(0, typePos);
		obj.childNodes.item(0).setAttribute("src", imgName+"_small"+fisheyemenu.imgType); 
	},
	imgLarge : function (obj) {
		...
		if (!(typePos>0)) return;
		var imgName = imgSrc.substr(0, typePos);
		obj.childNodes.item(0).setAttribute("src", imgName+fisheyemenu.imgType);
	}
}
